...
Thank you for your interest in contributing to Captain. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
cargo build --profile release-fast -p captain-cli
```

This cuts link time significantly (thin LTO, 8 codegen units, `opt-level=2`) while still producing a binary fast enough to run integration tests against. Use `--release` only for final binaries or maintainer release certification.
...
Run the focused gate for your change first, then the broader package tests that
cover the touched surface. `scripts/release-readiness.sh` is the required gate
before publication.

### Run Tests for a Single Crate

```bash
cargo test -p captain-kernel
cargo test -p captain-runtime
cargo test -p captain-memory
```
...
Run this locally before opening a pull request. Maintainers can dispatch the
GitHub Actions CI workflow manually when remote verification is needed.
...
Always run `cargo fmt` before committing. The local gate rejects unformatted code.
...
- **Naming**:
  - Types: `PascalCase` (e.g., `CaptainKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `captain-{name}` (kebab-case)
...
Captain is organized as a Cargo workspace. The root `Cargo.toml` is the source
of truth for current members; avoid duplicating volatile crate/tool totals in
documentation.

| Crate | Role |
|-------|------|
| `captain-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `captain-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `captain-runtime` | Agent loop, LLM drivers, built-in tools, detached run supervision, WASM sandbox, MCP client/server, A2A compatibility |
| `captain-hands` | Retained/frozen curated capability packages; not an active primary surface |
| `captain-extensions` | Retained/frozen integration registry and credential compatibility |
| `captain-graph` | Graph projection and relationship support used by runtime knowledge paths |
| `captain-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `captain-api` | REST/WS/SSE API (Axum 0.8), SPA dashboard, OpenAI-compatible `/v1/chat/completions`, agent-as-service routes |
| `captain-channels` | Active Telegram/Discord/Signal/Email adapters, frozen compatibility for non-core adapters, formatter, rate limiter |
| `captain-wire` | OFP (Captain Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `captain-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `captain-skills` | Skill system with bundled/generated skill governance, OpenClaw-format compatibility, prompt injection scanning |
| `captain-desktop` | Retained/frozen Tauri wrapper; active desktop paths are the TUI and Control web app |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `captain-runtime`, implemented on `CaptainKernel` in `captain-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.captain/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
```bash
captain agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/captain-channels/src/`. Each adapter implements the `ChannelAdapter` trait.
New non-core adapters must stay behind the frozen compatibility policy until the
Captain Core Excellence plan explicitly promotes them into the active channel
surface. Do not add setup prompts, default bridge startup, or model guidance for
a new channel without a release gate.

### Steps

1. Create a new file: `crates/captain-channels/src/myplatform.rs`
...
3. Register the module in `crates/captain-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/captain-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `captain-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/captain-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/captain-runtime/src/tool_runner.rs`.
...
3. **Test thoroughly**:
   - Run the focused gate for the changed surface and the broader package tests
     listed in the plan or PR.
...
```
Add frozen Matrix adapter scaffolding
...
- Open a [GitHub Issue](https://github.com/Vivien83/captain/issues) for questions, bugs, or feature requests.
