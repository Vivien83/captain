...
## Operating Existing APIs

When the user points Captain at an existing API provider, treat the OpenAPI document as the execution contract:

1. Read the official spec or generated docs before the first call.
2. Extract `servers`, operationId, method, path, auth scheme, required path/query/body params, date formats, pagination, and response schema.
3. Use the extracted contract to build the first request; do not discover required params by repeated 4xx errors when the spec is available.
4. Record stable discoveries as procedural memory: create or refine a skill with endpoint groups, exact request examples, safety classification, credentials handling, and verification steps.
5. Keep secrets out of the skill body. Reference environment variables or Captain skill `env_inject` requirements instead.
