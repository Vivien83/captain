...
## Agent-Ready API Workflow

When Captain is asked to operate a real third-party API, do not improvise from memory:

1. Locate the official docs, OpenAPI spec, Postman collection, SDK docs, or MCP server docs.
2. Extract the exact base URL, auth method, endpoint path, HTTP method, required path/query/body parameters, response shape, pagination, rate limits, and safety level.
3. Use secrets from Captain's vault/integration layer or a skill `env_inject` declaration. Never hardcode tokens into snippets, skills, memory, logs, or final answers.
4. Prefer a typed integration, skill-provided tool, official CLI, or official MCP server when one exists. Use ad-hoc `curl` or `execute_code` only after the contract is known.
5. For read-only metrics/status calls, make the first request with all required parameters from the spec. A missing-parameter 4xx should be a fallback diagnostic, not the discovery strategy.
6. After a successful non-trivial workflow, propose or refine a skill that records the endpoint, exact command/request shape, required parameters, safety gate, and verification command.
