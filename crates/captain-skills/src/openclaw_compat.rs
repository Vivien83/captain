...
pub struct SkillMdFrontmatter {
    ...
    /// Optional discovery family. Captain stores this as a `family:<id>` tag.
    pub family: Option<String>,
    /// Optional discovery tags from generated or hand-written SKILL.md files.
    pub tags: Vec<String>,
...
/// Result of converting a SKILL.md into Captain format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → captain_name).
...
fn skillmd_tags(frontmatter: &SkillMdFrontmatter, base_tags: &[&str]) -> Vec<String> {
    ...
    if let Some(family) = frontmatter.family.as_deref().map(str::trim) {
        if !family.is_empty() {
            let family_tag = format!("family:{family}");
            if !tags.iter().any(|existing| existing == &family_tag) {
                tags.push(family_tag);
            }
        }
    }
    tags
...
/// Full conversion of a SKILL.md directory to Captain format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to Captain tool definitions
        for cmd in &meta.commands {
            ...
            let captain_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_captain_tool(&cmd.name) {
                cmd.name.clone()
            } else {
    ...
    let manifest = SkillManifest {
        skill: SkillMeta {
            ...
            author: String::new(),
            license: String::new(),
            tags: skillmd_tags(&frontmatter, &["openclaw-compat", "prompt-only"]),
        },
...
/// Convert an in-memory SKILL.md string into Captain format.
///
...
pub fn convert_skillmd_str(name_hint: &str, content: &str) -> Result<ConvertedSkillMd, SkillError> {
    ...
    let manifest = SkillManifest {
        skill: SkillMeta {
            ...
            tags: skillmd_tags(&frontmatter, &["bundled", "prompt-only"]),
        },
        runtime: SkillRuntimeConfig {
            runtime_type,
            entry: String::new(),
...
/// Convert an OpenClaw Node.js skill directory into an Captain SkillManifest.
///
...
/// Write an Captain skill.toml manifest for an OpenClaw skill.
pub fn write_captain_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
...
# Git Helper instructions"#;
    ...
    #[test]
    fn test_convert_skillmd_str_preserves_family_metadata() {
        let content = r#"---
name: "smoke-check"
...
"#;
        let converted = convert_skillmd_str("fallback-name", content).unwrap();
        assert_eq!(converted.manifest.skill.name, "smoke-check");
        assert!(converted
            .manifest
            .skill
            .tags
