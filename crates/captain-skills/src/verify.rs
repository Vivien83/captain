...
impl SkillVerifier {
    ...
    pub fn security_scan(manifest: &SkillManifest) -> Vec<SkillWarning> {
        ...
        // Check for secret access via env_inject. Every entry maps an
        // arbitrary key from ~/.captain/secrets.env — which holds every
        // credential the user has stored — into the skill's process
        // environment, chosen entirely by the untrusted manifest with no
        // restriction tying it to the skill's declared purpose. This was
        // completely unchecked: a malicious skill could request any
        // credential and receive it silently. Every request is flagged,
        // not just ones matching a "looks sensitive" name pattern — the
        // file's whole purpose is credentials, so there's no safe subset
        // to exempt from the warning.
        for (secret_key, target_var) in &manifest.requirements.env_inject {
            warnings.push(SkillWarning {
                severity: WarningSeverity::Critical,
                message: format!(
                    "Skill requests secret '{secret_key}' from secrets.env, injected as '{target_var}'"
                ),
            });
        }
...
mod tests {
    ...
    /// Live finding: a skill declaring env_inject for any secrets.env key
    /// received it silently at runtime with zero warning anywhere in
    /// security_scan — a supply-chain credential-exfiltration vector via
    /// the ClawHub marketplace. Every declared key must now be flagged.
    #[test]
    fn test_security_scan_flags_env_inject_as_critical() {
        let manifest: SkillManifest = toml::from_str(
            r#"
            [skill]
            name = "innocuous-skill"
            [runtime]
            type = "python"
            entry = "main.py"
            [requirements]
        ...
        assert!(warnings
            .iter()
            .any(|w| w.severity == WarningSeverity::Critical
                && w.message.contains("GITHUB_TOKEN")
                && w.message.contains("GH_TOKEN")));
    }
