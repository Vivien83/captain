...
// ---------------------------------------------------------------------------
// Captain output types (TOML)
// ---------------------------------------------------------------------------

/// Captain config.toml structure for serialization.
#[derive(Serialize)]
struct CaptainConfig {
    default_model: CaptainModelConfig,
    memory: CaptainMemorySection,
    network: CaptainNetworkSection,
    #[serde(skip_serializing_if = "Option::is_none")]
    channels: Option<toml::Value>,
}

#[derive(Serialize)]
struct CaptainModelConfig {
    provider: String,
    model: String,
    api_key_env: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize)]
struct CaptainMemorySection {
    decay_rate: f32,
}

#[derive(Serialize)]
struct CaptainNetworkSection {
    listen_addr: String,
}
...
/// Map OpenClaw DM policy to Captain DM policy string.
...
/// Map OpenClaw group policy to Captain group policy string.
...
use captain_types::tool_compat::{is_known_captain_tool, map_tool_name};

/// Map OpenClaw tool profile to Captain capability tool list.
...
/// Map OpenClaw provider name to Captain provider name.
fn map_provider(openclaw_provider: &str) -> String {
    match openclaw_provider.to_lowercase().as_str() {
        ...
        // Additional Captain-supported providers and aliases
...
fn is_known_captain_provider_id(provider: &str) -> bool {
    matches!(
        provider,
        "anthropic"
            | "claude"
...
fn resolve_provider_with_models_context(
    ...
    let raw_is_known = is_known_captain_provider_id(&raw);
    let mapped_is_known = is_known_captain_provider_id(&mapped);
...
fn migrate_config_from_json(
    ...
    let of_config = CaptainConfig {
        ...
        memory: CaptainMemorySection { decay_rate: 0.05 },
        network: CaptainNetworkSection {
    ...
    let config_content = format!(
        "# Captain Agent OS configuration\n\
...
fn migrate_channels_from_json(
    ...
    // --- BlueBubbles (skip — no Captain adapter) ---
    if oc_channels.bluebubbles.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: "bluebubbles".to_string(),
            reason: "No Captain adapter available — consider using the iMessage channel instead"
                .to_string(),
        });
    }
    ...
    for key in oc_channels.other.keys() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: key.clone(),
            reason: format!("Unknown channel '{key}' — not mapped to any Captain adapter"),
        });
    }
            ...
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{id}': tool '{tool}' has no Captain equivalent and was skipped"
                    ));
                }
...
fn convert_agent_from_json(
    ...
    let system_prompt = entry
        ...
        .unwrap_or_else(|| {
            format!(
                "You are {display_name}, an AI agent running on the Captain Agent OS. You are helpful, concise, and accurate."
            )
        });
    ...
    toml_str.push_str("author = \"captain\"\n");
...
fn report_skipped_features(root: &OpenClawRoot, source: &Path, report: &mut MigrationReport) {
    ...
    if root.cron.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Config,
            name: "cron".to_string(),
            reason: "Cron job scheduling not yet supported — use Captain's ScheduleMode::Periodic instead".to_string(),
        });
    }
    ...
    if root.hooks.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Webhook hooks not supported — use Captain's event system instead".to_string(),
        });
    }
        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `captain skill install`".to_string(),
                });
            }
        }
    }
    ...
    if source.join("memory-search").join("index.db").exists() {
        report.skipped.push(SkippedItem {
            ...
            reason: "SQLite vector index not portable — Captain will rebuild embeddings"
                .to_string(),
        });
    }
    ...
    if root.session.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Session scope config differs — Captain uses per-agent sessions by default"
                .to_string(),
        });
    }
    ...
    if root.memory.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason:
                "Memory backend config not migrated — Captain uses SQLite with vector embeddings"
                    .to_string(),
        });
    }
}
        ...
        match *name {
            ...
            "bluebubbles" => {
                report.skipped.push(SkippedItem {
                    ...
                    name: "bluebubbles".to_string(),
                    reason:
                        "No Captain adapter available — consider using the iMessage channel instead"
                            .to_string(),
                });
            }
            _ => {}
    ...
    for entry in entries {
        ...
        match convert_legacy_agent(&agent_yaml, &agent_name) {
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{agent_name}': tool '{tool}' has no Captain equivalent and was skipped"
                    ));
                }
...
fn convert_legacy_agent(
    ...
    let system_prompt = oc.system_prompt.unwrap_or_else(|| {
        format!(
            "You are {}, an AI agent running on the Captain Agent OS. {}",
            oc.name,
            ...
            for entry in entries.flatten() {
                ...
                if has_package_json && has_index {
                    report.skipped.push(SkippedItem {
                        kind: ItemKind::Skill,
                        name: name.clone(),
                        reason: "Node.js skill — run with `captain skill install` after migration"
                            .to_string(),
                    });
                } else {
                    report.skipped.push(SkippedItem {
...
mod tests {
  ...
  agents: {
    ...
    fn test_is_known_captain_tool() {
        assert!(is_known_captain_tool("file_read"));
        assert!(is_known_captain_tool("shell_exec"));
        assert!(is_known_captain_tool("web_fetch"));
