...
pub fn validate_and_repair_with_stats(messages: &[Message]) -> (Vec<Message>, RepairStats) {
    let (cleaned, mut stats) = structural_repair(messages);

    // Phase 3: Merge consecutive same-role messages. This is a per-request
    // provider constraint (strict role alternation), NOT a history repair —
    // see repair_stored_session_with_stats for the stored-session variant.
    ...
    // Debug, not warn: per-request repairs are expected after context-economy
    // trimming (a trimmed ToolUse orphans its result on the request copy,
    // every autonomous tick). Persisted-session anomalies are surfaced
    // separately by repair_session_in_place and the compaction audit.
...
/// Structural repair only, safe for the STORED session: removes orphaned
/// ToolResults, empty messages and duplicates, reorders results and inserts
/// synthetic errors — but never merges consecutive same-role messages. Two
/// user turns in a row are legitimate history.
pub fn repair_stored_session_with_stats(messages: &[Message]) -> (Vec<Message>, RepairStats) {
    structural_repair(messages)
}

fn structural_repair(messages: &[Message]) -> (Vec<Message>, RepairStats) {
...
fn reorder_tool_results(messages: &mut Vec<Message>) -> usize {
    ...
    sorted_insertions.sort_by_key(|(idx, _)| std::cmp::Reverse(*idx));
...
fn insert_synthetic_results(messages: &mut Vec<Message>) -> usize {
    ...
    sorted.sort_by_key(|(idx, _)| std::cmp::Reverse(*idx));
