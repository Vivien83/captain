...
pub(crate) fn lmstudio_models() -> Vec<ModelCatalogEntry> {
...
mod tests {
    ...
    #[test]
    fn lmstudio_models_count_is_stable() {
        let models = lmstudio_models();

        assert_eq!(models.len(), 1);
        assert_eq!(models[0].provider, "lmstudio");
        assert_eq!(models[0].id, "lmstudio-local");
        assert!(models[0].aliases.is_empty());
    }

    #[test]
    fn lmstudio_local_contract_is_preserved() {
        let model = &lmstudio_models()[0];

        assert_eq!(model.display_name, "LM Studio Local Model");
