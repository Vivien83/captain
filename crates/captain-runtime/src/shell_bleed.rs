        ...
        for var_name in env_vars {
            ...
            if is_suspicious {
                warnings.push(ShellBleedWarning {
                    ...
                    suggestion: format!(
                        "Only read '{}' when it is injected from Captain's vault via a native integration or skill env_inject; never paste the raw secret into the script.",
                        var_name
                    ),
                });
            }
...
pub fn format_warnings(warnings: &[ShellBleedWarning]) -> String {
    ...
    output.push_str("Use secret_write/secret_read for vault state, then a native integration or skill [requirements.env_inject] for runtime access. Never paste raw secrets into scripts, commands, or .env files generated by the model.\n\n");
    output
}
