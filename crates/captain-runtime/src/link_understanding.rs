...
pub use captain_types::media::LinkConfig;
...
pub fn build_link_context(text: &str, config: &LinkConfig) -> Option<String> {
    ...
    context.push_str(
        "Use web_fetch for readable pages. For PDF/report/file URLs, use web_download then document_extract before summarizing or citing.\n",
    );
    Some(context)
}
