...
//!
//! MCP uses JSON-RPC 2.0 over stdio or HTTP+SSE. This module lets Captain
...
impl McpConnection {
    ...
    async fn send_request(
        ...
        match &mut self.transport {
            ...
            McpTransportHandle::Sse { client, url } => {
                ...
                if let Some(token) = sse_bearer_from_env(self.config.auth_token_env.as_deref())? {
    ...
    async fn send_notification(
        ...
        match &mut self.transport {
            ...
            McpTransportHandle::Sse { client, url } => {
                let mut request = client.post(url.as_str()).json(&notification);
                if let Some(token) = sse_bearer_from_env(self.config.auth_token_env.as_deref())? {
                    request = request.bearer_auth(token);
                }
...
fn sse_bearer_from_env(auth_token_env: Option<&str>) -> Result<Option<String>, String> {
    let Some(env_name) = auth_token_env.map(str::trim).filter(|s| !s.is_empty()) else {
        return Ok(None);
    };
    std::env::var(env_name)
        .map(Some)
        .map_err(|_| format!("MCP SSE auth_token_env '{env_name}' is not set"))
}
...
mod tests {
    ...
    fn test_mcp_transport_config_serde() {
        ...
        let sse_config = McpServerConfig {
            ...
            timeout_secs: 60,
            env: vec![],
            auth_token_env: Some("MCP_TEST_TOKEN".to_string()),
        };
        let json = serde_json::to_string(&sse_config).unwrap();
        let back: McpServerConfig = serde_json::from_str(&json).unwrap();
        assert_eq!(back.auth_token_env.as_deref(), Some("MCP_TEST_TOKEN"));
        match back.transport {
    ...
    #[test]
    fn sse_bearer_from_env_ignores_empty_config() {
        assert_eq!(sse_bearer_from_env(None).unwrap(), None);
        assert_eq!(sse_bearer_from_env(Some("")).unwrap(), None);
        assert_eq!(sse_bearer_from_env(Some("   ")).unwrap(), None);
    }
}
