...
use captain_types::model_catalog::{
    ...
    CODEX_BASE_URL, COHERE_BASE_URL, DEEPSEEK_BASE_URL, FIREWORKS_BASE_URL, GEMINI_BASE_URL,
...
fn provider_defaults(provider: &str) -> Option<ProviderDefaults> {
    match provider {
        ...
        "codex" | "openai-codex" => Some(ProviderDefaults {
            // Phase-k.1: subscription endpoint by default — most users come from
            // `codex login` (ChatGPT Plus session token), not an OPENAI_API_KEY.
            // Override `base_url` in config.toml to fall back to api.openai.com.
...
pub fn create_driver(config: &DriverConfig) -> Result<Arc<dyn LlmDriver>, LlmError> {
    ...
    // Codex — uses the dedicated Responses API driver authenticated via
    // OAuth (ChatGPT subscription). OpenAI API keys deliberately belong to
    // provider="openai"; do not silently mix the two auth paths.
    //
    // Phase O.1 (2026-04-26): the chatgpt.com/backend-api/codex endpoint
    // does NOT speak /chat/completions — it requires the Responses API
    // (`/responses` with `input[]`/`instructions` payload, originator and
    // ChatGPT-Account-ID headers). Reusing OpenAIDriver here caused 403→
    // 429 retry loops surfaced as "stuck on thinking" in the chat.
    if provider == "codex" || provider == "openai-codex" {
        // Phase-k.5: auto-refresh the access_token if within 120 s of expiry.
        let codex_session = crate::model_catalog::read_codex_credential_with_refresh();
        match codex_session {
            Some(token) => {
                let base_url = config
                    .base_url
            ...
            None => {
                if let Some(reason) = crate::model_catalog::codex_oauth_readiness_error() {
                    return Err(LlmError::AuthenticationFailed(reason));
                }
                return Err(LlmError::MissingApiKey(
                    "Aucune session Codex trouvée. Lance `captain login codex` (abonnement \
                     ChatGPT Plus) ou, depuis le chat, utilise `codex_login_start` puis \
                     `codex_login_poll`. Pour l'API OpenAI standard, utilise provider='openai', \
                     pas 'codex'."
                        .to_string(),
                ));
            }
...
mod tests {
    ...
    fn test_nvidia_provider_no_key_errors() {
        // NVIDIA NIM provider with no API key should error. Pass an explicit
        // empty key so this test stays deterministic on developer machines
        // that happen to export NVIDIA_API_KEY.
        let config = DriverConfig {
            provider: "nvidia".to_string(),
