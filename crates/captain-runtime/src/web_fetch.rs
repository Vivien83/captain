...
impl WebFetchEngine {
    ...
    ///
    /// B.6 — auto-follow of redirects is **disabled**. The fetch loop walks
    /// each `Location` header itself, re-runs `check_ssrf` on every hop,
    /// and rejects redirect chains that pivot to a private IP.
    /// `reqwest::redirect::Policy::none()` ensures the SSRF guard is never
    /// silently bypassed by a 301/302 to `169.254.169.254`.
    ...
    pub async fn fetch_with_options(
        ...
        // B.6 — Walk redirects manually. Each hop's destination is
        // re-validated by check_ssrf, so a public 301 → private 169.254...
        // pivot is rejected mid-chain instead of silently followed.
        const MAX_REDIRECTS: usize = 10;
        let mut current_url = url.to_string();
        let mut current_method = method_upper.clone();
        let mut current_body = body.map(str::to_string);
        let mut hops = 0usize;
        let resp = loop {
            let mut hop_req = match current_method.as_str() {
            ...
            if let Some(hdrs) = headers {
                for (k, v) in hdrs {
                    if let Some(val) = v.as_str() {
                        hop_req = hop_req.header(k.as_str(), val);
                    }
                }
            }
            if let Some(ref b) = current_body {
                if b.trim_start().starts_with('{') || b.trim_start().starts_with('[') {
                    hop_req = hop_req.header("Content-Type", "application/json");
                }
                hop_req = hop_req.body(b.clone());
            }

            let hop_resp = hop_req
                .send()
                .await
                .map_err(|e| format!("HTTP request failed: {e}"))?;

            let status_code = hop_resp.status();
            if !status_code.is_redirection() {
                break hop_resp;
            }
            // 3xx — extract Location, re-validate, advance.
            let next = hop_resp
                .headers()
                .get(reqwest::header::LOCATION)
                .and_then(|v| v.to_str().ok())
                .map(str::to_string)
                .ok_or_else(|| format!("HTTP {status_code} without a Location header"))?;
            let resolved = resolve_redirect(&current_url, &next);
            check_ssrf(&resolved)?;

            // 303 See Other forces a GET regardless of original method;
            // 301/302/307/308 keep the method (RFC 7231 §6.4).
            if status_code == reqwest::StatusCode::SEE_OTHER {
            ...
            if hops > MAX_REDIRECTS {
                return Err(format!(
                    "redirect chain exceeded {MAX_REDIRECTS} hops — refusing to follow"
                ));
            }
        };
...
/// B.6 — Resolve a `Location` header against the URL we just fetched.
///
/// The header may be absolute (`https://example.com/foo`), root-relative
/// (`/foo`), or path-relative (`foo/bar`). We need every flavour to land
/// on a fully-qualified URL so `check_ssrf` can re-validate it before the
/// next hop.
pub(crate) fn resolve_redirect(base: &str, location: &str) -> String {
    if location.starts_with("http://") || location.starts_with("https://") {
        return location.to_string();
    }
    // Extract the scheme + authority of `base`.
    ...
    if location.starts_with('/') {
        // Root-relative: scheme + authority + location
        format!("{scheme_authority}{location}")
    } else {
        // Path-relative: drop the last path segment of base, append location
        let path = &base[authority_end..];
        let last_slash = path.rfind('/').unwrap_or(0);
        format!("{scheme_authority}{}/{location}", &path[..last_slash])
    }
}
...
mod tests {
    ...
    /// B.6 — `Location: /foo` lands on the same scheme+host.
    #[test]
    fn resolve_redirect_root_relative() {
        let r = super::resolve_redirect("https://example.com/a/b", "/c");
        assert_eq!(r, "https://example.com/c");
    }

    /// B.6 — A bare relative path is appended to the parent directory.
    #[test]
    ...
    /// B.6 — Absolute Location passes through.
    #[test]
    fn resolve_redirect_absolute() {
        let r = super::resolve_redirect("https://example.com/a", "https://other.org/x");
        assert_eq!(r, "https://other.org/x");
    }

    /// B.6 — A redirect chain that pivots to a private IP must be
    /// rejected at the SSRF layer when the resolved URL is re-validated.
    #[test]
    fn resolve_redirect_then_ssrf_blocks_private_pivot() {
        // Public site issues `Location: http://169.254.169.254/...`
        let resolved = super::resolve_redirect(
            "https://public.example.com/",
            "http://169.254.169.254/latest/meta-data/",
        );
        assert_eq!(resolved, "http://169.254.169.254/latest/meta-data/");
        // The fetch loop calls check_ssrf on the resolved URL — it must
        // refuse the next hop.
        assert!(
            super::check_ssrf(&resolved).is_err(),
            "redirect to AWS IMDS must be blocked by check_ssrf"
        );
    }
}
