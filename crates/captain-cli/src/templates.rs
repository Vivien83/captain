...
/// 2. `~/.captain/agents/` (installed templates)
/// 3. `CAPTAIN_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects CAPTAIN_HOME)
    let of_home = if let Ok(h) = std::env::var("CAPTAIN_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".captain")
    } else {
        std::env::temp_dir().join(".captain")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("CAPTAIN_AGENTS_DIR") {
