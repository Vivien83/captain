...
fn full_tui_help_delegates_to_i18n_body() {
    ...
    assert!(fr.contains("/artifacts"));
    assert!(fr.contains("/runs"));
...
fn standalone_help_keeps_captain_chat_scope() {
    ...
    assert!(help.contains("/artifacts    — browse checksum-verified produced files"));
    assert!(help.contains("/runs         — summarize retained tool runs"));
...
fn standalone_help_is_localized_in_french() {
    ...
    assert!(help.contains("/artifacts    — parcourir les fichiers produits vérifiés"));
    assert!(help.contains("/runs         — résumer les exécutions d'outils conservées"));
