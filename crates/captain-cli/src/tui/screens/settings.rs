    ...
    if state.loading && state.providers.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(
                    crate::i18n::t("settings.loading_providers", crate::i18n::current())
                        .to_string(),
                    theme::dim_style(),
                ),
            ])),
    ...
    if state.loading && state.models.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled(format!("  {spinner} "), Style::default().fg(theme::CYAN)),
                Span::styled(
                    crate::i18n::t("settings.loading_models", crate::i18n::current()).to_string(),
                    theme::dim_style(),
                ),
            ])),
            chunks[1],
        ...
        f.render_widget(
            Paragraph::new(Span::styled(
                crate::i18n::t("settings.no_models", crate::i18n::current()).to_string(),
                theme::dim_style(),
            )),
            chunks[1],
        );
...
fn draw_tools(f: &mut Frame, area: Rect, state: &mut SettingsState) {
    ...
    if state.tools.is_empty() {
        f.render_widget(
            Paragraph::new(Span::styled(
                crate::i18n::t("settings.no_tools", crate::i18n::current()).to_string(),
                theme::dim_style(),
            )),
            chunks[1],
        );
