        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled(
                    format!(
                        "  {:<28} {:<16} {:<14} {:<6} {}",
                        "Session", "Agent", "Session ID", "Msgs", "Updated"
                    ),
                    theme::table_header(),
                ),
        ...
        let items: Vec<ListItem> = state
            ...
            .map(|&idx| {
                ...
                let id_short = if s.id.len() > 12 {
                    format!("{}\u{2026}", captain_types::truncate_str(&s.id, 12))
                } else {
                    s.id.clone()
                };
                ListItem::new(Line::from(vec![
                    Span::styled(
                        format!("  {:<28}", truncate(&s.label, 27)),
                        Style::default().fg(theme::CYAN),
                    ),
                    Span::styled(
                        format!(" {:<16}", truncate(&s.agent_name, 15)),
                        theme::dim_style(),
                    ),
                    Span::styled(format!(" {:<14}", id_short), theme::dim_style()),
                    Span::styled(
