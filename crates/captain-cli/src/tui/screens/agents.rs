...
pub struct InProcessAgent {
    pub id: captain_types::agent::AgentId,
    pub name: String,
    pub state: String,
    pub provider: String,
    pub model: String,
...
impl AgentSelectState {
    ...
    pub fn load_available_skills(&mut self, assigned: Vec<String>, available: Vec<String>) {
        self.available_skills = checked_name_options(&assigned, available);
        self.skill_cursor = 0;
    }

    pub fn load_available_mcp_servers(&mut self, assigned: Vec<String>, available: Vec<String>) {
        self.available_mcp = checked_name_options(&assigned, available);
        self.mcp_cursor = 0;
    }
    ...
    pub fn load_inprocess_agents(&mut self, kernel: &captain_kernel::CaptainKernel) {
...
fn agent_list_indices(state: &AgentSelectState) -> Vec<usize> {
...
fn agent_list_row(state: &AgentSelectState, combined: usize) -> Option<AgentListRow> {
    let daemon_count = state.daemon_agents.len();
    if combined < daemon_count {
        let a = state.daemon_agents.get(combined)?;
        Some(AgentListRow {
            id: a.id.clone(),
            name: a.name.clone(),
            state: a.state.clone(),
        ...
        let a = state
            .inprocess_agents
            .get(combined.checked_sub(daemon_count)?)?;
        Some(AgentListRow {
            id: a.id.to_string(),
            name: a.name.clone(),
            state: a.state.clone(),
...
fn agent_list_item(row: AgentListRow) -> ListItem<'static> {
    let (badge, badge_style) = theme::state_badge(&row.state);
    ListItem::new(Line::from(vec![
        Span::styled(format!("  {:<5}", badge), badge_style),
        Span::styled(
            format!(" {:<18}", truncate(&row.name, 17)),
            Style::default().fg(theme::CYAN),
        ),
        Span::styled(
            format!(
                " {:<24}",
                truncate(&format!("{}/{}", row.provider, row.model), 23)
            ),
            Style::default().fg(theme::YELLOW),
        ),
        Span::styled(format!(" {}", truncate(&row.id, 12)), theme::dim_style()),
    ]))
}

fn create_agent_list_item() -> ListItem<'static> {
...
fn agent_list_items(state: &AgentSelectState) -> Vec<ListItem<'static>> {
    let mut items: Vec<ListItem<'static>> = agent_list_indices(state)
        .into_iter()
        .filter_map(|combined| agent_list_row(state, combined))
        .map(agent_list_item)
        .collect();
    items.push(create_agent_list_item());
    items
}

fn render_agent_search(f: &mut Frame, area: Rect, state: &AgentSelectState) {
...
fn render_agent_list_header(f: &mut Frame, area: Rect) {
    f.render_widget(
...
fn render_agent_list_status(f: &mut Frame, area: Rect, status_msg: &str) {
...
fn agent_list_hints(state: &AgentSelectState) -> &'static str {
...
fn draw_agent_list_full(f: &mut Frame, area: Rect, state: &mut AgentSelectState) {
    ...
    let search_height = u16::from(state.search_active || !state.search_query.is_empty());
    ...
    render_agent_search(f, chunks[0], state);
    render_agent_list_header(f, chunks[1]);

    let list = List::new(agent_list_items(state))
        .highlight_style(theme::selected_style())
        .highlight_symbol("> ");
    f.render_stateful_widget(list, chunks[2], &mut state.list);

    render_agent_list_status(f, chunks[2], &state.status_msg);
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            agent_list_hints(state),
            theme::hint_style(),
        )])),
        chunks[3],
    );
...
fn detail_field(label: &'static str, value: impl Into<String>, style: Style) -> Line<'static> {
    Line::from(vec![Span::raw(label), Span::styled(value.into(), style)])
}

fn push_agent_detail_metadata(lines: &mut Vec<Line<'static>>, detail: &AgentDetail) {
...
fn push_agent_detail_access(lines: &mut Vec<Line<'static>>, detail: &AgentDetail) {
...
fn agent_detail_lines(detail: &AgentDetail) -> Vec<Line<'static>> {
    let (badge, badge_style) = theme::state_badge(&detail.state);
    let mut lines = vec![
        Line::from(""),
        detail_field("  ID:       ", detail.id.clone(), theme::dim_style()),
        detail_field(
            "  Name:     ",
            detail.name.clone(),
            Style::default()
    ...
    push_agent_detail_metadata(&mut lines, detail);
    push_agent_detail_access(&mut lines, detail);
    lines
}
...
fn draw_detail(f: &mut Frame, area: Rect, state: &AgentSelectState) {
    ...
    match &state.detail {
        Some(detail) => f.render_widget(Paragraph::new(agent_detail_lines(detail)), chunks[0]),
...
fn checked_name_options(assigned: &[String], available: Vec<String>) -> Vec<(String, bool)> {
    available
        .into_iter()
        .map(|name| {
...
mod tests {
    ...
    fn state_with_daemon_and_local_agent() -> (AgentSelectState, captain_types::agent::AgentId) {
        let mut state = AgentSelectState::new();
        state.daemon_agents.push(DaemonAgent {
            id: "daemon-agent-123456".to_string(),
            name: "Captain".to_string(),
            state: "running".to_string(),
            provider: "codex".to_string(),
            model: "gpt-5.5".to_string(),
        });
        let local_id = captain_types::agent::AgentId::new();
        state.inprocess_agents.push(InProcessAgent {
            id: local_id,
    ...
    #[test]
    fn agent_list_indices_follow_filter_when_query_is_active() {
        let (mut state, _) = state_with_daemon_and_local_agent();

        assert_eq!(agent_list_indices(&state), vec![0, 1]);

        state.search_query = "worker".to_string();
        state.rebuild_filter();

        assert_eq!(agent_list_indices(&state), vec![1]);
    }

    #[test]
    fn agent_list_row_maps_daemon_and_inprocess_entries() {
        let (state, local_id) = state_with_daemon_and_local_agent();

        assert_eq!(
            agent_list_row(&state, 0),
            Some(AgentListRow {
                id: "daemon-agent-123456".to_string(),
                name: "Captain".to_string(),
                state: "running".to_string(),
        ...
        assert_eq!(agent_list_row(&state, 2), None);
    }

    #[test]
    fn agent_list_hints_reflect_search_mode() {
        let (mut state, _) = state_with_daemon_and_local_agent();

        assert!(agent_list_hints(&state).contains("[/] Search"));

        state.search_active = true;

        assert!(agent_list_hints(&state).contains("Cancel search"));
    }

    #[test]
    fn agent_detail_lines_include_optional_runtime_metadata() {
        let detail = AgentDetail {
            ...
            tags: vec!["ops".to_string(), "prod".to_string()],
            capabilities: vec!["shell".to_string()],
            parent: Some("captain".to_string()),
            children: vec!["child".to_string()],
            skills: vec!["memory".to_string()],
            skills_mode: "allow".to_string(),
            mcp_servers: vec!["filesystem".to_string()],
            mcp_servers_mode: "allow".to_string(),
        };
        let text = agent_detail_lines(&detail)
            .iter()
            .map(line_text)
            .collect::<Vec<_>>()
            .join("\n");

        assert!(text.contains("Created:  today"));
        assert!(text.contains("Tags:     ops, prod"));
        assert!(text.contains("Caps:     shell"));
        assert!(text.contains("Parent:   captain"));
        assert!(text.contains("Children: child"));
        assert!(text.contains("Skills:   memory"));
        assert!(text.contains("MCP:      filesystem"));
    }

    #[test]
    fn agent_detail_lines_show_all_access_fallbacks() {
        let detail = AgentDetail {
            id: "agent-1".to_string(),
            name: "Worker".to_string(),
            state: "idle".to_string(),
            ...
            mcp_servers_mode: "all".to_string(),
            ..AgentDetail::default()
        };
        let text = agent_detail_lines(&detail)
            .iter()
        ...
        assert!(text.contains("Skills:   [All skills]"));
        assert!(text.contains("MCP:      [All servers]"));
    }

    #[test]
    fn load_available_skills_marks_assigned_names_and_resets_cursor() {
        let mut state = AgentSelectState::new();
        state.skill_cursor = 7;

        state.load_available_skills(
            vec!["memory".to_string(), "browser".to_string()],
            vec![
                "shell".to_string(),
                "memory".to_string(),
                "browser".to_string(),
        ...
        assert_eq!(state.skill_cursor, 0);
    }

    #[test]
    fn load_available_mcp_servers_marks_assigned_names_and_resets_cursor() {
        let mut state = AgentSelectState::new();
        state.mcp_cursor = 3;

        state.load_available_mcp_servers(
