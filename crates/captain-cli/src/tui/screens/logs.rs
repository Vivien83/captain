...
pub fn draw(f: &mut Frame, area: Rect, state: &mut LogsState) {
    ...
    if state.search_mode {
        ...
        let auto_badge = if state.auto_refresh {
            Span::styled(
                crate::i18n::t("logs.auto_on", lang).to_string(),
                Style::default().fg(theme::GREEN),
            )
        } else {
            Span::styled(
                crate::i18n::t("logs.auto_off", lang).to_string(),
                theme::dim_style(),
            )
        };
        let search_hint = if state.search_buf.is_empty() {
            ...
            Span::styled(
                crate::i18n::t("logs.filter", lang).replace("{query}", &state.search_buf),
        ...
        f.render_widget(
            Paragraph::new(vec![
                Line::from(vec![
                    Span::styled(
                        crate::i18n::t("logs.level_label", lang).to_string(),
                        theme::dim_style(),
                    ),
                    Span::styled(
                    ...
                    Span::styled(
                        crate::i18n::t("logs.entries_count", lang)
                            .replace("{count}", &state.filtered.len().to_string()),
                        theme::dim_style(),
                    ),
                    auto_badge,
                    search_hint,
    ...
    if state.loading && state.entries.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(
                    crate::i18n::t("logs.loading", lang).to_string(),
                    theme::dim_style(),
                ),
            ])),
            chunks[1],
        ...
        f.render_widget(
            Paragraph::new(Span::styled(
                crate::i18n::t("logs.no_match", lang).to_string(),
                theme::dim_style(),
            )),
            chunks[1],
        );
