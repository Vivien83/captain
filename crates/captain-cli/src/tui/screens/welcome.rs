//! Welcome screen: compact identity, daemon/provider status, mode selection.
...
// ── Branding ─────────────────────────────────────────────────────────────────
// The terminal identity stays deliberately compact; graphical surfaces render
// the full crown asset.

const LOGO_HEIGHT: u16 = branding::CAPTAIN_HEIGHT;
const LOGO_MIN_WIDTH: u16 = branding::CAPTAIN_WIDTH + 4;

const COMPACT_LOGO: &str = "CAPTAIN";
...
impl WelcomeState {
    ...
    fn rebuild_menu(&mut self) {
        ...
        let t = |k: &str| i18n::t(k, lang).to_string();

        self.menu_items.clear();
        if self.daemon_url.is_some() {
            self.menu_items.push(MenuItem {
                label: t("welcome.menu.connect_daemon"),
                hint: t("welcome.menu.connect_daemon.hint"),
...
pub fn draw(f: &mut Frame, area: Rect, state: &mut WelcomeState) {
    ...
    // The wordmark is one terminal row in both layouts.
    ...
    if compact {
        ...
        f.render_widget(branding::captain_paragraph(), chunks[1]);
    }
    ...
    if state.detecting {
        ...
        if let Some(ref url) = state.daemon_url {
            ...
            status_lines.push(Line::from(vec![
                ...
                Span::styled(
                    crate::i18n::t("welcome.no_daemon", lang).to_string(),
                    theme::dim_style(),
                ),
            ]));
        }
        ...
        if let Some((provider, env_var)) = detect_provider() {
            ...
            status_lines.push(Line::from(vec![
                ...
                Span::styled(
                    crate::i18n::t("welcome.no_provider", lang).to_string(),
                    Style::default().fg(theme::YELLOW),
                ),
            ]));
            status_lines.push(Line::from(vec![Span::styled(
                "  Run 'captain init' to get started",
                theme::hint_style(),
            )]));
        }
        ...
        let items: Vec<ListItem> = state
            ...
            .map(|item| {
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled(item.hint.clone(), theme::dim_style()),
                ]))
            })
            .collect();
