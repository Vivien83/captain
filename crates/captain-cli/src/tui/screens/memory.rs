    ...
    if state.loading {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(
                    crate::i18n::t("memory.loading_agents", lang).to_string(),
                    theme::dim_style(),
                ),
            ])),
            chunks[1],
        ...
        f.render_widget(
            Paragraph::new(Span::styled(
                crate::i18n::t("memory.no_agents", lang).to_string(),
                theme::dim_style(),
            )),
            chunks[1],
        );
        ...
        let items: Vec<ListItem> = state
            ...
            .map(|a| {
                let id_short = if a.id.len() > 12 {
                    format!("{}\u{2026}", captain_types::truncate_str(&a.id, 12))
                } else {
                    a.id.clone()
                };
    ...
    if state.loading {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled(format!("  {spinner} "), Style::default().fg(theme::CYAN)),
                Span::styled(
                    crate::i18n::t("memory.loading", lang).to_string(),
                    theme::dim_style(),
                ),
            ])),
            chunks[1],
        ...
        let items: Vec<ListItem> = state
            ...
            .map(|kv| {
                let val_display = if kv.value.len() > 40 {
                    format!("{}\u{2026}", captain_types::truncate_str(&kv.value, 39))
                } else {
                    kv.value.clone()
                };
                ListItem::new(Line::from(vec![
...
fn draw_edit(f: &mut Frame, area: Rect, state: &MemoryState) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            crate::i18n::t("memory.key_label", crate::i18n::current()).to_string(),
            key_label_style,
        )])),
        chunks[2],
    );
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            crate::i18n::t("memory.value_label", crate::i18n::current()).to_string(),
            val_label_style,
        )])),
        chunks[4],
    );
