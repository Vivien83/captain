//! MCP (Model Context Protocol) server for Captain.
//!
//! Exposes running agents as MCP tools over JSON-RPC 2.0 stdio.
//! Each agent becomes a callable tool named `captain_agent_{name}`.
//!
...
impl McpBackend {
    ...
    fn send_message(&self, agent_id: &str, message: &str) -> Result<String, String> {
        match self {
            ...
            McpBackend::InProcess { kernel, rt } => {
                let aid: captain_types::agent::AgentId =
    ...
    /// Find agent ID by tool name (strip `captain_agent_` prefix, match by name).
    fn resolve_tool_agent(&self, tool_name: &str) -> Option<String> {
        let agent_name = tool_name.strip_prefix("captain_agent_")?.replace('_', "-");
        ...
        let agent_name_underscore = tool_name.strip_prefix("captain_agent_")?;
        for (id, name, _) in &agents {
    ...
    match method {
        "initialize" => {
            let result = json!({
                ...
                "serverInfo": {
                    "name": "captain",
                    "version": crate::cli_runtime::captain_version()
                }
            });
        ...
        "tools/list" => {
            ...
            let tools: Vec<Value> = agents
                ...
                .map(|(_, name, description)| {
                    let tool_name = format!("captain_agent_{}", name.replace('-', "_"));
                    let desc = if description.is_empty() {
                        format!("Send a message to Captain agent '{name}'")
                    } else {
                        description.clone()
                    };
                    json!({
...
mod tests {
    ...
    fn test_handle_initialize() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "captain");
    }
