...
use captain_types::memory::{MemoryFilter, MemoryFragment, MemoryId, MemorySource};
use chrono::Utc;
...
impl SemanticStore {
    ...
    pub fn recall_with_embedding(
        ...
        for row_result in rows {
            let (
                ...
                access_count,
                embedding_bytes,
            ) = row_result.map_err(|e| CaptainError::Memory(e.to_string()))?;

            let id = uuid::Uuid::parse_str(&id_str)
                .map(MemoryId)
                .map_err(|e| CaptainError::Memory(e.to_string()))?;
            let agent_id = uuid::Uuid::parse_str(&agent_str)
                .map(captain_types::agent::AgentId)
                .map_err(|e| CaptainError::Memory(e.to_string()))?;
        ...
        // Update access counts for returned memories in a single batched statement,
        // instead of one UPDATE per fragment while holding the shared connection mutex.
        if !fragments.is_empty() {
            let placeholders = std::iter::repeat_n("?", fragments.len())
                .collect::<Vec<_>>()
                .join(", ");
            let sql = format!(
                "UPDATE memories SET access_count = access_count + 1, accessed_at = ? WHERE id IN ({placeholders})"
            );
            let now = Utc::now().to_rfc3339();
            ...
            for frag in &fragments {
                update_params.push(Box::new(frag.id.0.to_string()));
            }
            let update_param_refs: Vec<&dyn rusqlite::types::ToSql> =
                update_params.iter().map(|p| p.as_ref()).collect();
            if let Err(e) = conn.execute(&sql, update_param_refs.as_slice()) {
                warn!("Failed to update access counts for recalled memories: {e}");
            }
        }
    ...
    pub fn forget(&self, id: MemoryId) -> CaptainResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
...
mod tests {
    ...
    #[test]
    fn test_recall_batches_access_count_updates() {
        let store = setup();
        let agent_id = AgentId::new();
        for i in 0..5 {
            store
        ...
        // First recall should bump access_count for every returned fragment,
        // via a single batched UPDATE rather than one execute() per fragment.
        let results = store.recall("Batch memory", 10, None).unwrap();
        assert_eq!(results.len(), 5);

        let results_after = store.recall("Batch memory", 10, None).unwrap();
        assert_eq!(results_after.len(), 5);
        for frag in &results_after {
            assert!(
                frag.access_count >= 1,
                "expected access_count to be incremented for {:?}",
                frag.id
            );
        }
    }
