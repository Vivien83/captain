...
const AUTO_SESSION_LABEL_MAX_CHARS: usize = 72;
const LEGACY_TUI_SESSION_NAMESPACE: uuid::Uuid = uuid::Uuid::from_bytes([
    0x63, 0x61, 0x70, 0x74, 0x61, 0x69, 0x6e, 0x2d, 0x74, 0x75, 0x69, 0x2d, 0x76, 0x32, 0x00, 0x01,
]);

/// Return the stable session ID used when importing one legacy TUI JSON file.
///
/// `source_key` is the path relative to `~/.captain/sessions`, using `/` as
/// separator. UUID v5 makes repeated boots idempotent without mutating the
/// historical JSON mirror.
pub fn legacy_tui_session_id(source_key: &str) -> SessionId {
    SessionId(uuid::Uuid::new_v5(
        &LEGACY_TUI_SESSION_NAMESPACE,
        source_key.as_bytes(),
    ))
}

fn inferred_session_label(messages: &[Message]) -> Option<String> {
    messages
        .iter()
        .filter(|message| message.role == Role::User)
        .filter_map(|message| {
            let text = message.content.text_content();
            normalize_session_label_candidate(&text)
        })
        .next()
}

fn normalize_session_label_candidate(input: &str) -> Option<String> {
    ...
    for word in title.split_whitespace() {
        let separator = usize::from(!shortened.is_empty());
        if shortened.chars().count() + separator + word.chars().count()
            > AUTO_SESSION_LABEL_MAX_CHARS
        {
            break;
        }
        if separator == 1 {
    ...
    if shortened.is_empty() {
        shortened = title.chars().take(AUTO_SESSION_LABEL_MAX_CHARS).collect();
    }

    let mut characters = shortened.chars();
    let first = characters.next()?;
    Some(first.to_uppercase().collect::<String>() + characters.as_str())
}

fn resolved_session_label(label: Option<String>, messages: &[Message]) -> Option<String> {
    label.or_else(|| inferred_session_label(messages))
}
...
impl SessionStore {
        ...
        match result {
            Ok((agent_str, messages_blob, tokens, label)) => {
                ...
                let messages: Vec<Message> = rmp_serde::from_slice(&messages_blob)
                    .map_err(|e| CaptainError::Serialization(e.to_string()))?;
                let label = resolved_session_label(label, &messages);
                Ok(Some(Session {
                    id: session_id,
                    agent_id,
                    messages,
    ...
    pub fn save_session(&self, session: &Session) -> CaptainResult<()> {
        ...
        conn.execute(
            "INSERT INTO sessions (id, agent_id, messages, context_window_tokens, label, created_at, updated_at)
             ...
             ON CONFLICT(id) DO UPDATE SET messages = ?3, context_window_tokens = ?4, label = COALESCE(label, ?5), updated_at = ?6",
            rusqlite::params![
    ...
    /// Import a historical session once while preserving its original dates.
    ///
    /// Existing rows are never updated: a session may already have continued
    /// through another surface by the time a later boot scans the legacy TUI
    /// mirror again.
    pub fn import_session_if_absent(
        ...
        let messages_blob = rmp_serde::to_vec_named(&session.messages)
            .map_err(|e| CaptainError::Serialization(e.to_string()))?;
        let label = resolved_session_label(session.label.clone(), &session.messages);
        let created_at = unix_seconds_rfc3339(created_at_secs);
        let updated_at = unix_seconds_rfc3339(updated_at_secs.max(created_at_secs));
        let inserted = conn
            .execute(
...
impl SessionStore {
    ...
    pub fn list_agent_sessions(&self, agent_id: AgentId) -> CaptainResult<Vec<serde_json::Value>> {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT id, messages, context_window_tokens, created_at, updated_at, label \
        ...
        let rows = stmt
            .query_map(rusqlite::params![agent_id.0.to_string()], |row| {
                let session_id: String = row.get(0)?;
                let messages_blob: Vec<u8> = row.get(1)?;
                let context_window_tokens: i64 = row.get(2)?;
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> CaptainResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    ) -> CaptainResult<CanonicalSession> {
        let mut canonical = self.load_canonical(agent_id)?;
                ...
                for msg in compacting {
                    ...
                    if !text.is_empty() {
                        ...
                        let truncated = if text.len() > 200 {
                            format!("{}...", captain_types::truncate_str(&text, 200))
                        } else {
                            text
                        };
    ...
    ) -> CaptainResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> CaptainResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
                        ...
                        match block {
                            ...
                            ContentBlock::Thinking { thinking, .. } => {
                                text_parts.push(format!(
                                    "[thinking: {}]",
                                    captain_types::truncate_str(thinking, 200)
                                ));
                            }
...
mod tests {
    ...
    #[test]
    fn legacy_tui_session_ids_are_stable_and_source_scoped() {
        let first = legacy_tui_session_id("daemon-captain/1710000000.json");
        assert_eq!(
            first,
            legacy_tui_session_id("daemon-captain/1710000000.json")
        );
        assert_ne!(
            first,
            legacy_tui_session_id("daemon-captain/1710000001.json")
        );
    }

    #[test]
    fn historical_import_is_idempotent_and_never_overwrites_continued_session() {
        let store = setup();
        let agent_id = AgentId::new();
        let session_id = legacy_tui_session_id("captain/history.json");
        let historical = Session {
            id: session_id,
            agent_id,
            messages: vec![Message::user("historical request")],
            context_window_tokens: 0,
            label: None,
        };

        assert!(store
            .import_session_if_absent(&historical, 1_700_000_000, 1_700_000_010)
            .unwrap());

        let mut continued = store.get_session(session_id).unwrap().unwrap();
        continued
            .messages
            .push(Message::assistant("continued from web"));
        store.save_session(&continued).unwrap();
        ...
        assert_eq!(loaded.label.as_deref(), Some("Historical request"));
    }
    ...
    #[test]
    fn session_label_uses_first_meaningful_user_message() {
        let store = setup();
        let agent_id = AgentId::new();
        let mut session = store.create_session(agent_id).unwrap();
        session.messages.extend([
            ...
            Message::user("Peux-tu organiser les documents administratifs du couple ?"),
        ]);
        ...
        assert_eq!(
            ...
            Some("Organiser les documents administratifs du couple")
        );
        let listed = store.list_agent_sessions(agent_id).unwrap();
        assert_eq!(
            listed[0].get("label").and_then(|value| value.as_str()),
            loaded.label.as_deref()
        );
    }

    #[test]
    fn explicit_session_label_is_never_overwritten() {
        let store = setup();
        let agent_id = AgentId::new();
        let mut session = store
            .create_session_with_label(agent_id, Some("Dossier famille"))
            .unwrap();
        session.messages.push(Message::user(
            "Prépare un classement complètement différent",
        ));
        store.save_session(&session).unwrap();

        let mut stale_copy = session.clone();
        stale_copy.label = None;
        stale_copy
            .messages
            .push(Message::assistant("Le classement est prêt."));
        store.save_session(&stale_copy).unwrap();

        let loaded = store.get_session(session.id).unwrap().unwrap();
        assert_eq!(loaded.label.as_deref(), Some("Dossier famille"));
    }

    #[test]
    fn legacy_unlabelled_session_gets_a_display_label_without_a_new_turn() {
        let store = setup();
        let agent_id = AgentId::new();
        let session = store.create_session(agent_id).unwrap();
        let messages = vec![Message::user("Audit complet du service Inventory")];
        let blob = rmp_serde::to_vec_named(&messages).unwrap();
        {
            let conn = store.conn.lock().unwrap();
            conn.execute(
                "UPDATE sessions SET messages = ?1, label = NULL WHERE id = ?2",
                rusqlite::params![blob, session.id.0.to_string()],
            )
            .unwrap();
        }
        ...
        assert_eq!(
            loaded.label.as_deref(),
            Some("Audit complet du service Inventory")
        );
        let listed = store.list_agent_sessions(agent_id).unwrap();
        assert_eq!(
            listed[0].get("label").and_then(|value| value.as_str()),
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(captain_types::message::Message::user("Hello"));
        session
            .messages
            .push(captain_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
