...
pub struct UsageRecord {
    ...
    /// Input tokens served from provider prompt cache.
    #[serde(default)]
    pub cached_input_tokens: u64,
    /// Tokens used to populate provider prompt cache.
    #[serde(default)]
...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> CaptainResult<()> {
        let conn = self
            .conn
            .lock()
        ...
        conn.execute(
            "INSERT INTO usage_events (id, agent_id, timestamp, model, input_tokens, output_tokens, cached_input_tokens, cache_creation_tokens, cost_usd, tool_calls)
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> CaptainResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> CaptainResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> CaptainResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> CaptainResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> CaptainResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> CaptainResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> CaptainResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_daily_breakdown(&self, days: u32) -> CaptainResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> CaptainResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> CaptainResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> CaptainResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CaptainError::Internal(e.to_string()))?;
...
mod tests {
    ...
    #[test]
    fn test_query_daily_breakdown_bound_param_matches_expected_window() {
        let store = setup();
        let agent_id = AgentId::new();
        ...
        // The `days` value is now passed as a bound parameter (concatenated into
        // the datetime modifier by SQLite itself) rather than interpolated into
        // the SQL text. Verify the query still returns today's event within the
        // window and excludes it once the window no longer covers "now".
        let breakdown = store.query_daily_breakdown(7).unwrap();
        assert_eq!(breakdown.len(), 1);
        assert!((breakdown[0].cost_usd - 0.02).abs() < 0.0001);
        assert_eq!(breakdown[0].calls, 1);

        // A 0-day window ('now', '-0 days') still includes "now", so use a
        // sanity check that a larger window keeps returning the same single row.
        let breakdown_wide = store.query_daily_breakdown(30).unwrap();
        assert_eq!(breakdown_wide.len(), 1);
    }
