...
//!
//! Boots the Captain kernel, binds to a random localhost port, and runs the
//! API server on a background thread with its own tokio runtime.

use captain_api::server::build_router;
...
impl ServerHandle {
    ...
    pub fn shutdown(mut self) {
            ...
            .is_ok()
        {
            ...
            info!("Captain embedded server stopped");
        }
    }
}
...
pub fn start_server() -> Result<ServerHandle, Box<dyn std::error::Error>> {
    ...
    // Without this, API keys stored in ~/.captain/.env are invisible to
    // the kernel's provider detection and credential resolver.
    load_dotenv_files();

    // Boot kernel (sync — no tokio needed)
    let kernel = CaptainKernel::boot(None)?;
    ...
    info!("Captain embedded server bound to http://127.0.0.1:{port}");
...
async fn run_embedded_server(
    ...
    info!("Captain embedded server listening on http://{listen_addr}");
...
/// Load ~/.captain/.env and ~/.captain/secrets.env into the process environment.
/// System env vars take priority — existing vars are NOT overridden.
fn load_dotenv_files() {
    let home = if let Ok(h) = std::env::var("CAPTAIN_HOME") {
        ...
        std::path::PathBuf::from(user_home).join(".captain")
    };
