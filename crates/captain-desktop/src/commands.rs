...
/// `~/.captain/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = captain_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.captain/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = captain_home().join("skills");
...
/// Open the Captain config directory (`~/.captain/`) in the OS file manager.
#[tauri::command]
...
/// Open the Captain logs directory (`~/.captain/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = captain_home().join("logs");
