//! Captain Hands — curated autonomous capability packages.
//!
...
impl Default for HandAgentConfig {
    fn default() -> Self {
        Self {
            name: String::new(),
...
/// Auto-fills agent.name/description/system_prompt from hand-level fields if empty.
pub fn parse_hand_toml(content: &str) -> Result<HandDefinition, toml::de::Error> {
    let mut def = if let Ok(d) = toml::from_str::<HandDefinition>(content) {
        d
    } else {
        let wrapper: HandTomlWrapper = toml::from_str(content)?;
        wrapper.hand
    };
    // Auto-fill agent config from hand-level fields when [agent] section is omitted
    if def.agent.name.is_empty() {
        def.agent.name = def.name.clone();
    }
    if def.agent.description.is_empty() {
        def.agent.description = def.description.clone();
    }
    if def.agent.system_prompt.is_empty() {
        def.agent.system_prompt = format!(
            "You are {}, an autonomous agent. {}. Be helpful, proactive, and respond in the user's language.",
...
pub struct HandDefinition {
    ...
    /// Agent manifest template (auto-derived from name/description if omitted).
    #[serde(default)]
