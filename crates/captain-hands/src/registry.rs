...
impl HandRegistry {
    ...
    /// Scan a directory for hand definitions (subdirectories containing HAND.toml).
    /// Skips hands already registered (bundled take precedence).
    pub fn scan_directory(&self, hands_dir: &std::path::Path) -> usize {
        let entries = match std::fs::read_dir(hands_dir) {
            Ok(e) => e,
            Err(e) => {
                debug!(path = %hands_dir.display(), error = %e, "Cannot scan hands directory");
                return 0;
            }
        };
        let mut count = 0;
        for entry in entries.flatten() {
            ...
            match self.install_from_path(&path) {
                Ok(def) => {
                    info!(hand = %def.id, name = %def.name, "Loaded custom hand from filesystem");
                    count += 1;
                }
                Err(HandError::AlreadyActive(_)) => {} // bundled takes precedence
                Err(e) => {
                    warn!(path = %path.display(), error = %e, "Failed to load custom hand");
                }
            }
        }
        count
...
mod tests {
    ...
    fn env_var_requirement_check() {
        std::env::set_var("CAPTAIN_TEST_HAND_REQ", "test_value");
        let req = HandRequirement {
            key: "test".to_string(),
            label: "test".to_string(),
            requirement_type: RequirementType::EnvVar,
            check_value: "CAPTAIN_TEST_HAND_REQ".to_string(),
            description: None,
            optional: false,
            install: None,
        };
        ...
        let req_missing = HandRequirement {
            ...
            label: "test".to_string(),
            requirement_type: RequirementType::EnvVar,
            check_value: "CAPTAIN_NONEXISTENT_VAR_12345".to_string(),
            description: None,
            optional: false,
            install: None,
        };
        assert!(!check_requirement(&req_missing));
        std::env::remove_var("CAPTAIN_TEST_HAND_REQ");
    }
    ...
    fn readiness_active_hand_degraded() {
        ...
        // Browser hand requires a Chromium-based browser. Activate it — if the
