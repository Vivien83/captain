...
mod tests {
    ...
    fn parse_collector_hand() {
        ...
        assert!(def.tools.contains(&"web_research_batch".to_string()));
        assert!(def.tools.contains(&"web_download".to_string()));
        assert!(def.tools.contains(&"document_extract".to_string()));
        assert!(def.tools.contains(&"document_pipeline".to_string()));
        assert!(def.tools.contains(&"browser_batch".to_string()));
        assert!(!def.settings.is_empty());
        assert!(!def.dashboard.metrics.is_empty());
    }
    ...
    fn parse_browser_hand() {
        ...
        assert_eq!(def.category, crate::HandCategory::Productivity);
        assert!(def.skill_content.is_some());
        assert!(!def.requires.is_empty()); // requires Chromium/Chrome for native CDP
        ...
        assert!(def.tools.contains(&"browser_keys".to_string()));
        assert!(def.tools.contains(&"browser_select".to_string()));
        assert!(def.tools.contains(&"browser_hover".to_string()));
