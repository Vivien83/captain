...
### Captain Native Tool Pattern
Use typed Captain rails before shell or ad hoc scripts:

1. `web_research_batch` for broad discovery with compact fetched previews.
2. `web_fetch` for a specific readable web page.
3. `web_download` for PDFs, reports, CSV/JSON datasets, whitepapers and files.
4. `document_extract` immediately after `web_download` before summarizing or citing the file.
5. `browser_batch` for JS-rendered pages, forms, login/session flows, screenshots, and downloads.
6. `document_create` / `document_pipeline` when the final deliverable is a PDF, DOCX, HTML or Markdown report.

A PDF/report is not citable just because it appeared in search results. It is
citable only after its content was actually read through `document_extract` or a
browser extraction path.
...
Before delivering a research answer or report, verify:

- Are primary sources present where they should be?
- Are any claims supported only by a single weak or commercial source?
- Are statistics traced to original reports/datasets when possible?
- Are contradictions or uncertainty disclosed?
- Does every citation correspond to a source actually read or extracted?
