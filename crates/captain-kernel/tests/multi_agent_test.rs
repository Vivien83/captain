...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p captain-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("captain-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = CaptainKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  CAPTAIN MULTI-AGENT FLEET TEST");
