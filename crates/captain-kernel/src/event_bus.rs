...
use captain_types::event::{Event, EventTarget};
use dashmap::DashMap;
...
///
/// Phase O.2: derives Clone so background components (e.g. the
/// auto-memorize commit notifier) can hold their own handle without
/// forcing every caller to switch to `Arc<EventBus>`. All inner state
/// is already shareable: `broadcast::Sender` is cheap to clone,
/// `DashMap` is internally Arc-based, and the history is wrapped in
/// `Arc<RwLock<…>>`.
#[derive(Clone)]
pub struct EventBus {
