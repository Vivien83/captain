//! Configuration loading from `~/.captain/config.toml` with defaults.
//!
                ...
                Ok(mut root_value) => {
                    ...
                    match root_value.try_into::<KernelConfig>() {
                        Ok(mut config) => {
                            ...
                            // CAPTAIN_HOME always wins over a stored `home_dir`/`data_dir`,
                            // matching captain_home()'s documented priority. Without this, a
                            // config.toml copied/restored into a different CAPTAIN_HOME
                            // (e.g. for an isolated test, or a relocated install) keeps
                            // pointing kernel file operations (daemon.json, PID lock, and
                            // critically the memory database) at the original directory it
                            // was copied from — risking concurrent access to the same
                            // database from two Captain instances.
                            if let Ok(env_home) = std::env::var("CAPTAIN_HOME") {
                                let env_home = PathBuf::from(env_home);
                                config.data_dir = env_home.join("data");
                                config.home_dir = env_home;
                            }
                            // v3.14-config.b.3 — silent reconcile so
                            // new versions surface their fields in
                            // the user's config without overwriting
                            // any edit. Best-effort: a failure here
                            // must never prevent boot.
                            match captain_types::config_template::reconcile_file(
                                &config_path,
                                ...
                                Ok(added) if !added.is_empty() => {
                                    info!(
                                        ...
                                        count = added.len(),
                                        keys = ?added,
                                        "config reconcile: added missing top-level sections"
                                    );
                                }
                                Ok(_) => {}
                                Err(e) => {
                                    tracing::warn!(
                                        error = %e,
                                        path = %config_path.display(),
                                        "config reconcile failed — continuing without"
                                    );
                                }
                            }
                            return config;
...
///
/// Respects `CAPTAIN_HOME` env var (e.g. `CAPTAIN_HOME=/opt/captain`).
pub fn default_config_path() -> PathBuf {
    captain_home().join("config.toml")
}

/// Get the Captain home directory.
///
/// Priority: `CAPTAIN_HOME` env var > `~/.captain`. On first boot after
/// a legacy rebrand, the old home directory is renamed in place so user
/// data survives without a manual copy.
pub fn captain_home() -> PathBuf {
    if let Ok(home) = std::env::var("CAPTAIN_HOME") {
        return PathBuf::from(home);
    }
    let home = dirs::home_dir().unwrap_or_else(std::env::temp_dir);
    let new_dir = home.join(".captain");
    let legacy = home.join(".horaclaw");
        ...
        match std::fs::rename(&legacy, &new_dir) {
            Ok(()) => {
                tracing::warn!(
                    ...
                    new = %new_dir.display(),
                    "Migrated legacy Captain home to ~/.captain"
                );
            }
            Err(e) => {
                tracing::warn!(
                    error = %e,
                    "Could not migrate legacy Captain home; falling back to ~/.captain"
                );
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_captain_home_env_overrides_stored_home_dir_and_data_dir() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("config.toml");
        std::fs::write(
            &config_path,
            r#"
home_dir = "/some/other/machine/.captain"
data_dir = "/some/other/machine/.captain/data"
"#,
        )
        .unwrap();

        let previous = std::env::var("CAPTAIN_HOME").ok();
        std::env::set_var("CAPTAIN_HOME", dir.path());
