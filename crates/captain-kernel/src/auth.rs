...
//! to Captain users with roles, then enforces permission checks on actions.

use captain_types::agent::UserId;
use captain_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Captain user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Captain UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> CaptainResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| CaptainError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(CaptainError::AuthDenied(format!(
