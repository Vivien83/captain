...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> CaptainResult<()> {
        // Hourly check
    ...
    pub fn budget_status(&self, budget: &captain_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> CaptainResult<UsageSummary> {
        self.store.query_summary(agent_id)
    }

    /// Get usage grouped by model.
    pub fn get_by_model(&self) -> CaptainResult<Vec<ModelUsage>> {
    ...
    /// Build the catalog lookup key used for pricing.
    ///
    /// Runtime drivers often carry provider-scoped models as bare IDs
    /// (`gpt-5.5` with provider `codex`). The catalog stores subscription
    /// Codex models under `codex/...` with zero API price, so using only the
    /// bare model can fall back to generic OpenAI pricing and overstate cost.
    pub fn catalog_pricing_model_id(provider: &str, model: &str) -> String {
    ...
    pub fn cleanup(&self, days: u32) -> CaptainResult<usize> {
...
fn estimate_cost_rates(model: &str) -> (f64, f64) {
    ...
    // ── Xiaomi MiMo (OpenRouter) ─────────────────────────────────
    if model.contains("mimo-v2-flash") || model.contains("mimo-v2-flash") {
        return (0.09, 0.29);
    }
    if model.contains("mimo-v2-omni") {
        return (0.40, 2.0);
    }
    if model.contains("mimo-v2-pro") || model.contains("mimo") {
        return (1.0, 3.0);
    }
...
mod tests {
    ...
    #[test]
    fn test_catalog_pricing_model_id_prefixes_bare_provider_model() {
        assert_eq!(
            MeteringEngine::catalog_pricing_model_id("codex", "gpt-5.5"),
            "codex/gpt-5.5"
        );
        assert_eq!(
            MeteringEngine::catalog_pricing_model_id("codex", "codex/gpt-5.5"),
            "codex/gpt-5.5"
        );
        assert_eq!(
            MeteringEngine::catalog_pricing_model_id("", "gpt-5.5"),
            "gpt-5.5"
        );
    }

    #[test]
    fn test_codex_subscription_pricing_uses_zero_catalog_row() {
        let catalog = captain_runtime::model_catalog::ModelCatalog::new();
        let model = MeteringEngine::catalog_pricing_model_id("codex", "gpt-5.5");
        let cost =
            MeteringEngine::estimate_cost_with_catalog(&catalog, &model, 1_000_000, 1_000_000);
        assert_eq!(cost, 0.0);
    }
