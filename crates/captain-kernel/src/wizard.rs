...
impl SetupWizard {
    ...
    pub fn build_plan(intent: AgentIntent) -> SetupPlan {
        ...
        // Add web/research source intake tools if web/network capability is needed.
        ...
        let system_prompt = format!(
            "You are {name}, an AI agent running inside the Captain Agent OS.\n\
             \n\
             YOUR TASK: {task}\n\
             \n\
             APPROACH:\n\
    ...
    fn tool_hints_for(tools: &[String]) -> String {
        ...
        if has("web_download") || has("document_extract") {
            hints.push("- For PDFs/reports/files, use web_download then document_extract before summarizing or citing.");
        }
        if has("browser_navigate") {
            hints.push("- Use browser_batch or browser_navigate/click/type/keys/select/hover/read_page to interact with websites.");
        }
        if has("file_read") {
...
mod tests {
    ...
    fn test_web_tools_auto_added() {
        ...
        assert!(plan
            ...
            .capabilities
            .tools
            .contains(&"web_research_batch".to_string()));
        assert!(plan
            .manifest
            .capabilities
            .tools
            .contains(&"web_download".to_string()));
        assert!(plan
            .manifest
            .capabilities
            .tools
            .contains(&"document_extract".to_string()));
    }
    ...
    fn test_browser_tools_auto_added() {
        ...
        assert!(plan
            ...
            .capabilities
            .tools
            .contains(&"browser_batch".to_string()));
        assert!(plan
            .manifest
            .capabilities
            .tools
            .contains(&"browser_keys".to_string()));
        assert!(plan
            .manifest
            .capabilities
            .tools
