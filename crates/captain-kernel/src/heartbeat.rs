...
//!
//! The heartbeat monitor runs as a background tokio task. It monitors autonomous
//! agents that declare a heartbeat interval and agents already marked Crashed.
//! Reactive chat agents are allowed to be idle; inactivity alone is not a crash.
//!
...
use captain_types::agent::{AgentId, AgentState};
use chrono::Utc;
use dashmap::DashMap;
...
pub fn check_agents(registry: &AgentRegistry, config: &HeartbeatConfig) -> Vec<HeartbeatStatus> {
    ...
    for entry_ref in registry.list() {
        // Check autonomous Running agents (for unresponsiveness) and Crashed
        // agents (for recovery). Reactive chat agents can sleep indefinitely:
        // their `last_active` is user activity, not a process heartbeat.
        let autonomous_cfg = entry_ref.manifest.autonomous.as_ref();
        match entry_ref.state {
            AgentState::Running if autonomous_cfg.is_some() => {}
            AgentState::Crashed => {}
            _ => continue,
        }
        ...
        let timeout_secs = autonomous_cfg
...
mod tests {
    ...
    fn test_entry(
        ...
        manifest.autonomous = autonomous;
    ...
    #[test]
    fn reactive_idle_agent_is_not_a_heartbeat_failure() {
        ...
        assert!(
            statuses.is_empty(),
            "reactive chat agents may sleep without being treated as crashed"
        );
    }

    #[test]
    fn autonomous_idle_agent_is_still_monitored() {
        let registry = AgentRegistry::new();
        registry
            .register(test_entry(
                "autonomous",
                AgentState::Running,
                Some(AutonomousConfig::default()),
                Utc::now() - chrono::Duration::seconds(600),
            ))
            .unwrap();
    ...
    #[test]
    fn crashed_agent_is_still_recovery_candidate() {
        let registry = AgentRegistry::new();
        registry
            .register(test_entry("crashed", AgentState::Crashed, None, Utc::now()))
            .unwrap();
