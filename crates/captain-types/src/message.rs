...
pub enum ContentBlock {
    ...
    Thinking {
        ...
        thinking: String,
        /// Provider-specific metadata required to continue a reasoning turn
        /// without exposing the raw chain-of-thought.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        provider_metadata: Option<serde_json::Value>,
    },
...
pub enum StopReason {
    ...
    MaxTokens,
    /// The provider ended the response while the turn was still in progress.
    Incomplete,
...
pub struct TokenUsage {
    ...
    /// Subset of `input_tokens` that the provider served from its prompt
    /// cache. Reported as 0 when the provider doesn't expose the metric or
    /// when nothing was cached. Used for observability of cache hit ratios.
    #[serde(default)]
    pub cached_input_tokens: u64,
    /// Tokens that the provider charged at a *premium* to populate its
    /// cache for future reads (Anthropic only — `cache_creation_input_tokens`).
    /// 0 elsewhere or on cache hits.
    #[serde(default)]
...
impl TokenUsage {
    ...
    /// Cache hit ratio over the input prompt — useful for telemetry.
    /// Returns 0.0 when `input_tokens` is 0 to avoid div-by-zero.
