...
//! ensuring they ship with every Captain build with zero filesystem dependencies.
...
pub fn bundled_integrations() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        // ── Memory (1) ─────────────────────────────────────────────────────
        ("mempalace", include_str!("../integrations/mempalace.toml")),
    ]
}
...
mod tests {
    ...
    fn all_have_transport() {
        for (id, content) in bundled_integrations() {
            let t: IntegrationTemplate = toml::from_str(content)
                .unwrap_or_else(|e| panic!("Failed to parse '{}': {}", id, e));
            // All bundled integrations use stdio transport
            match &t.transport {
