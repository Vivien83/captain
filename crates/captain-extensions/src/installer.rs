...
//!
//! Handles the complete flow: template lookup → credential persistence/resolution →
...
pub fn install_integration(
    ...
    // 2. Store provided keys in the canonical resolver-backed store.
    let mut persisted_keys = HashSet::new();
    for (key, value) in provided_keys {
        if resolver.has_credential(key) {
            persisted_keys.insert(key.clone());
            continue;
        }
        if let Err(e) = resolver.store_credential(key, value) {
            warn!("Could not persist credential {}: {}", key, e);
        }
        if resolver.has_credential(key) {
            persisted_keys.insert(key.clone());
        }
    ...
    // Only keys that are actually resolvable count as present. A caller that
    // merely passed a key in-memory must not get a false Ready status.
...
# Place this in ~/.captain/integrations/ or use `captain add --custom <path>`
...
3. Run: captain add my-integration --key=<your-key>
"""
"#;
...
mod tests {
    ...
    fn install_with_key() {
        ...
        assert_eq!(result.status, IntegrationStatus::Ready);
        assert!(resolver.has_credential("NOTION_API_KEY"));
    }
