//! Workspace-scoped lightweight adapter for durable Hub/Node tool runs.
//!
//! It reviews and executes only the narrow local surface advertised by a
//! Captain Node. Critical shell commands remain locally denied: a remote
//! approval is never converted into a critical shell permit.
...
use crate::tool_dispatch::ToolResult;
...
pub async fn execute_local_node_tool(
    ...
    let result = crate::tool_dispatch::execute_tool(
        request.tool_use_id,
        request.tool_name,
        request.input,
        &workspace_root,
...
pub fn local_node_tool_effect(
    ...
    Some(crate::effect::classify(tool_name, input))
}
...
fn review_node_shell(
    ...
    match crate::shell_guard::review(command, &policy) {
        Ok(()) => Ok(()),
        Err(crate::shell_guard::ShellReviewRejection::Critical) => {
        ...
        Err(crate::shell_guard::ShellReviewRejection::Denied) => {
            Err(LocalNodeToolRejection::permanent(
...
fn finalize_local_node_output(
    ...
    let (sanitized, retention_redacted) =
        crate::output_security::sanitize_for_retention(&virtualized).map_err(|_| {
            LocalNodeToolRejection::permanent(
