...
//!
//! Allows any OpenAI-compatible client library to talk to Captain agents.
//! The `model` field resolves to an agent (by name, UUID, or `captain:<name>`),
...
use captain_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "captain:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("captain:") {
...
async fn stream_response(
    ...
    let (mut rx, _handle, _user_input_tx) = state
        .kernel
    ...
    let stream_metric =
        crate::stream_metrics::StreamMetricHandle::start(agent_id.to_string(), "openai_compat");
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("captain:{}", e.name),
            object: "model",
            created,
            owned_by: "captain".to_string(),
        })
        .collect();
