//! Composition adapter between the durable Node worker and its local tool engine.

use crate::{
    AuthorizedNodeRun, NodeReviewedTool, NodeRunCancellation, NodeToolDriver,
    NodeToolExecutionOutput, NodeToolReview, NodeWorkerError,
};
...
pub struct NodeLocalToolDriver {
    exec_policy: ExecPolicy,
}

impl NodeLocalToolDriver {
    pub fn new(exec_policy: ExecPolicy) -> Self {
        Self { exec_policy }
    }
}

impl NodeToolDriver for NodeLocalToolDriver {
...
impl fmt::Debug for NodeLocalToolDriver {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("NodeLocalToolDriver")
