...
pub struct FlockAdapter {
    ...
    webhook_port: u16,
    /// Flock user IDs allowed to address the bot (empty = deny all,
    /// `["*"]` = allow all). Match is exact against `message.from` (e.g. `"u:user123"`).
...
impl FlockAdapter {
    ...
    /// * `allowed_users` - Flock user IDs authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(bot_token: String, webhook_port: u16, allowed_users: Vec<String>) -> Self {
...
fn parse_flock_event(
    ...
    if !crate::rbac::is_authorized(allowed_users, from) {
        debug!("Flock: ignoring event from unauthorised user {from} (allowed_users gate)");
        return None;
    }
...
impl ChannelAdapter for FlockAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        tokio::spawn(async move {
            ...
            let allowed_users_shared = Arc::new(allowed_users);
            ...
            let app = axum::Router::new().route(
                ...
                axum::routing::post({
                    ...
                    let allowed_users = Arc::clone(&allowed_users_shared);
                    move |body: axum::extract::Json<serde_json::Value>| {
                        ...
                        async move {
                            ...
                            if let Some(msg) = parse_flock_event(&body, &user_id, &allowed_users) {
...
mod tests {
    ...
    fn test_flock_adapter_creation() {
        let adapter = FlockAdapter::new("test-bot-token".to_string(), 8181, vec!["*".to_string()]);
    ...
    fn test_flock_webhook_port() {
        let adapter = FlockAdapter::new("token".to_string(), 7777, vec!["*".to_string()]);
    ...
    fn test_parse_flock_event_message() {
        ...
        let msg = parse_flock_event(&event, "u:bot001", &["*".to_string()]).unwrap();
    ...
    fn test_parse_flock_event_command() {
        ...
        let msg = parse_flock_event(&event, "u:bot001-different", &["*".to_string()]).unwrap();
        match &msg.content {
    ...
    fn test_parse_flock_event_skip_bot() {
        ...
        let msg = parse_flock_event(&event, "u:bot001", &["*".to_string()]);
        assert!(msg.is_none());
    }
    ...
    /// B.8 — empty allowed_users denies every sender.
    #[test]
    fn test_parse_flock_event_rbac_empty_denies() {
        let event = serde_json::json!({
            "name": "chat.receiveMessage",
            "message": {
                "text": "hi",
        ...
        assert!(parse_flock_event(&event, "u:bot001", &[]).is_none());
    }

    /// B.8 — explicit list authorises only listed users.
    #[test]
    fn test_parse_flock_event_rbac_explicit_list() {
        ...
        assert!(parse_flock_event(&alice, "u:bot001", &allowed).is_some());
        assert!(parse_flock_event(&mallory, "u:bot001", &allowed).is_none());
    }
