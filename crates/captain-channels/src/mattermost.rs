//! Mattermost channel adapter for the Captain channel bridge.
//!
...
pub struct MattermostAdapter {
    ...
    /// Mattermost user IDs allowed to address the bot (empty = deny all, ["*"] = allow all).
...
fn parse_mattermost_event(
    ...
    if !crate::rbac::is_authorized(allowed_users, user_id) {
        debug!("Mattermost: ignoring post from unauthorised user {user_id} (allowed_users gate)");
        return None;
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let should_reconnect = 'inner: loop {
                    ...
                    if let Some(channel_msg) = parse_mattermost_event(
                        &payload,
                        &bot_id_guard,
                        &allowed_users,
                        &allowed_channels,
...
mod tests {
    ...
    #[test]
    fn test_parse_mattermost_event_rbac_empty_denies() {
    ...
    fn test_parse_mattermost_event_basic() {
        ...
        let msg = parse_mattermost_event(&event, &bot_id, &["*".to_string()], &[]).unwrap();
    ...
    fn test_parse_mattermost_event_dm() {
        ...
        let msg = parse_mattermost_event(&event, &None, &["*".to_string()], &[]).unwrap();
        assert!(!msg.is_group);
    }
    ...
    fn test_parse_mattermost_event_skips_bot() {
        ...
        let event = serde_json::json!({
            ...
            "data": {
                "post": serde_json::to_string(&post).unwrap(),
                "channel_type": "O",
                "sender_name": "captain-bot"
            }
        });

        let bot_id = Some("bot-123".to_string());
        let msg = parse_mattermost_event(&event, &bot_id, &["*".to_string()], &[]);
        assert!(msg.is_none());
    }
    ...
    fn test_parse_mattermost_event_channel_filter() {
        ...
        let msg = parse_mattermost_event(
            &event,
            &None,
            &["*".to_string()],
            &["ch-111".to_string(), "ch-222".to_string()],
        );
        assert!(msg.is_none());

        // In allowed channels
        let msg =
            parse_mattermost_event(&event, &None, &["*".to_string()], &["ch-789".to_string()]);
        assert!(msg.is_some());
    }
    ...
    fn test_parse_mattermost_event_non_posted() {
        ...
        let msg = parse_mattermost_event(&event, &None, &["*".to_string()], &[]);
        assert!(msg.is_none());
    }
