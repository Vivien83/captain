...
pub struct ThreemaAdapter {
    ...
    webhook_port: u16,
    /// Threema IDs allowed to address the bot (empty = deny all,
...
impl ThreemaAdapter {
    ...
    /// * `allowed_users` - Threema IDs authorised to address the bot
...
fn parse_threema_webhook(
    ...
    if !crate::rbac::is_authorized(allowed_users, from) {
        debug!("Threema: ignoring webhook from unauthorised id {from} (allowed_users gate)");
        return None;
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(msg) = parse_threema_webhook(&payload, &own_id, &allowed_users) {
...
mod tests {
    ...
    #[test]
    fn test_parse_threema_webhook_rbac_empty_denies() {
        let mut payload = HashMap::new();
        payload.insert("from".to_string(), "ALICE001".to_string());
        payload.insert("text".to_string(), "hi".to_string());
        assert!(parse_threema_webhook(&payload, "*MYGATEW", &[]).is_none());
    }

    /// B.8 — explicit list authorises only listed Threema IDs.
    #[test]
    fn test_parse_threema_webhook_rbac_explicit_list() {
        let mut alice = HashMap::new();
        alice.insert("from".to_string(), "ALICE001".to_string());
        alice.insert("text".to_string(), "hi".to_string());
        let mut mallory = HashMap::new();
        mallory.insert("from".to_string(), "MALLORY1".to_string());
        mallory.insert("text".to_string(), "drop".to_string());
        let allowed = vec!["ALICE001".to_string()];
        assert!(parse_threema_webhook(&alice, "*MYGATEW", &allowed).is_some());
        assert!(parse_threema_webhook(&mallory, "*MYGATEW", &allowed).is_none());
    }
    ...
    fn test_parse_threema_webhook_basic() {
        ...
        let msg = parse_threema_webhook(&payload, "*MYGATEW", &["*".to_string()]).unwrap();
    ...
    fn test_parse_threema_webhook_skip_self() {
        ...
        let msg = parse_threema_webhook(&payload, "*MYGATEW", &["*".to_string()]);
        assert!(msg.is_none());
    }
