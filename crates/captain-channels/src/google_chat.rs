...
pub struct GoogleChatAdapter {
    ...
    /// Google Chat sender resource names allowed to address the bot
    /// (empty = deny all, `["*"]` = allow all). Match is exact against
    /// `message.sender.name` (e.g. `"users/12345"`).
...
impl GoogleChatAdapter {
    ...
    /// * `allowed_users` - Sender resource names authorised to address the bot
            ...
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    if !crate::rbac::is_authorized(&allowed_users, sender_id) {
                        warn!("Google Chat: denied sender {sender_id}");
                        return;
                    }
...
mod tests {
    ...
    fn test_google_chat_allowed_spaces() {
        ...
        let open = GoogleChatAdapter::new(
            r#"{"access_token":"tok"}"#.to_string(),
            vec![],
            8090,
            vec!["*".to_string()],
    ...
    fn test_google_chat_invalid_key() {
        let adapter =
            GoogleChatAdapter::new("not-json".to_string(), vec![], 8092, vec!["*".to_string()]);
    ...
    #[test]
    fn test_google_chat_rbac_empty_denies() {
        let adapter = GoogleChatAdapter::new(
            r#"{"access_token":"tok"}"#.to_string(),
            vec![],
            8093,
    ...
    #[test]
    fn test_google_chat_rbac_explicit_list() {
        let adapter = GoogleChatAdapter::new(
            r#"{"access_token":"tok"}"#.to_string(),
            vec![],
            8094,
