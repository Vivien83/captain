...
pub struct TwistAdapter {
    ...
    /// Twist user IDs allowed to address the bot (empty = deny all,
                ...
                for channel_id in &channels_to_poll {
                    ...
                    for thread in &threads {
                        ...
                        for comment in &comments {
                            ...
                            if !crate::rbac::is_authorized(&allowed_users, &creator) {
                                debug!(
                                    "Twist: ignoring comment from unauthorised creator {creator} (allowed_users gate)"
                                );
                                continue;
                            }
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies every creator.
    #[test]
    fn test_twist_rbac_empty_denies() {
        let adapter = TwistAdapter::new("tok".to_string(), "ws1".to_string(), vec![], vec![]);
        assert!(adapter.allowed_users.is_empty());
        assert!(!crate::rbac::is_authorized(&adapter.allowed_users, "12345"));
    }

    /// B.8 — explicit list authorises only listed creators.
    #[test]
    fn test_twist_rbac_explicit_list() {
        let adapter = TwistAdapter::new(
            "tok".to_string(),
            "ws1".to_string(),
            vec![],
        ...
        assert!(crate::rbac::is_authorized(&adapter.allowed_users, "12345"));
        assert!(!crate::rbac::is_authorized(&adapter.allowed_users, "67890"));
    }
