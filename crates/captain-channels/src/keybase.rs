...
pub struct KeybaseAdapter {
    ...
    /// Keybase usernames allowed to address the bot (empty = deny all,
...
impl KeybaseAdapter {
    ...
    /// * `allowed_users` - Usernames authorised to address the bot
            ...
            loop {
                ...
                for conv in &conversations {
                    ...
                    for msg_wrapper in &messages {
                        ...
                        if !crate::rbac::is_authorized(&allowed_users, sender_username) {
                            debug!(
                                "Keybase: ignoring message from unauthorised user {sender_username} (allowed_users gate)"
                            );
                            continue;
                        }
...
mod tests {
    ...
    #[test]
    fn test_keybase_rbac_empty_denies() {
        let adapter = KeybaseAdapter::new("bot".to_string(), "key".to_string(), vec![], vec![]);
    ...
    #[test]
    fn test_keybase_rbac_explicit_list() {
        let adapter = KeybaseAdapter::new(
            "bot".to_string(),
            "key".to_string(),
            vec![],
