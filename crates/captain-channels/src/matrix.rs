...
pub struct MatrixAdapter {
    ...
    /// Bot's user ID (e.g., "@captain:matrix.org").
    user_id: String,
    /// SECURITY: Access token is zeroized on drop.
    access_token: Zeroizing<String>,
    /// HTTP client.
    client: reqwest::Client,
    /// Matrix user IDs allowed to address the bot (empty = deny all, ["*"] = allow all).
                    ...
                    for (room_id, room_data) in rooms {
                        ...
                        if let Some(events) = room_data["timeline"]["events"].as_array() {
                            for event in events {
                                ...
                                // `["*"]` opts back into permissive.
                                if !crate::rbac::is_authorized(&allowed_users, sender) {
                                    debug!(
                                        "Matrix: ignoring message from unauthorised sender {sender} (allowed_users gate)"
                                    );
                                    continue;
                                }
...
mod tests {
    ...
    /// B.8 — empty allowed_users must deny.
    #[test]
    fn test_matrix_allowed_users_empty_denies() {
        assert!(!crate::rbac::is_authorized(&[], "@alice:example.org"));
    }

    /// `["*"]` opts back into permissive; explicit list filters.
    #[test]
    fn test_matrix_allowed_users_wildcard_and_explicit() {
        let any: Vec<String> = vec!["*".to_string()];
        assert!(crate::rbac::is_authorized(&any, "@anyone:matrix.org"));
        let only_alice = vec!["@alice:matrix.org".to_string()];
        assert!(crate::rbac::is_authorized(&only_alice, "@alice:matrix.org"));
