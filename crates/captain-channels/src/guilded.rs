...
pub struct GuildedAdapter {
    ...
    /// Guilded user IDs allowed to address the bot (empty = deny all,
...
impl GuildedAdapter {
    ...
    /// * `allowed_users` - User IDs authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(bot_token: String, server_ids: Vec<String>, allowed_users: Vec<String>) -> Self {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let should_reconnect = loop {
                    ...
                    if !crate::rbac::is_authorized(&allowed_users, created_by) {
                        debug!(
                            "Guilded: ignoring message from unauthorised user {created_by} (allowed_users gate)"
                        );
                        continue;
                    }
...
mod tests {
    ...
    fn test_guilded_allowed_servers() {
        ...
        let open = GuildedAdapter::new("tok".to_string(), vec![], vec!["*".to_string()]);
    ...
    #[test]
    fn test_guilded_rbac_empty_denies() {
        let adapter = GuildedAdapter::new("tok".to_string(), vec![], vec![]);
    ...
    #[test]
    fn test_guilded_rbac_explicit_list() {
        let adapter =
            GuildedAdapter::new("tok".to_string(), vec![], vec!["user-alice".to_string()]);
        assert!(crate::rbac::is_authorized(
            &adapter.allowed_users,
            "user-alice"
        ));
