//! IRC channel adapter for the Captain channel bridge.
//!
...
/// Maximum number of alternate nicknames to try on ERR_NICKNAMEINUSE (433)
/// before giving up on this connection attempt.
const MAX_NICK_ATTEMPTS: u32 = 5;

/// Derive the next alternate nickname to try after a 433 (nickname taken).
///
/// Each attempt must produce a *different* candidate so repeated rejections
/// converge instead of retrying the exact same (already-rejected) nick
/// forever: `nick_`, `nick__`, then `nick1`, `nick2`, ...
fn alt_nick_for_attempt(base: &str, attempt: u32) -> String {
    match attempt {
        1 => format!("{base}_"),
        2 => format!("{base}__"),
        n => format!("{base}{}", n - 2),
...
pub struct IrcAdapter {
    ...
    /// IRC nicks allowed to address the bot (empty = deny all, ["*"] = allow all).
    allowed_users: Vec<String>,
    /// IRC channels to join (e.g., ["#captain", "#bots"]).
    channels: Vec<String>,
...
fn parse_privmsg(
    ...
    if !crate::rbac::is_authorized(allowed_users, sender_nick) {
        debug!("IRC: ignoring PRIVMSG from unauthorised nick {sender_nick} (allowed_users gate)");
        return None;
    }
        ...
        *self.write_tx.write().await = Some(write_cmd_tx.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                registration.push_str(&format!("USER {nick} 0 * :Captain Bot\r\n"));
                    ...
                    tokio::select! {
                        line_result = lines.next_line() => {
                            ...
                            match parsed.command.as_str() {
                                ...
                                "PRIVMSG" => {
                                    if let Some(msg) =
                                        parse_privmsg(&parsed, &nick_clone, &allowed_users)
                                    {
                                        debug!(
                                            "IRC message from {}: {:?}",
                                ...
                                // The alternate must change on every attempt
                                // (nick_, nick__, nick1, ...) or a server that
                                // also rejects the alternate sends the exact
                                // same 433 forever, looping indefinitely.
                                "433" => {
                                    nick_attempts += 1;
                                    if nick_attempts > MAX_NICK_ATTEMPTS {
                                        error!(
                                            "IRC: giving up after {MAX_NICK_ATTEMPTS} nickname attempts for '{nick_clone}' — server keeps rejecting alternates"
                                        );
                                        break 'inner true;
                                    }
                                    let alt_nick = alt_nick_for_attempt(&nick_clone, nick_attempts);
                                    warn!(
                                        "IRC: nickname '{nick_clone}' is already in use, trying '{alt_nick}' (attempt {nick_attempts}/{MAX_NICK_ATTEMPTS})"
                                    );
                        ...
                        _ = shutdown_rx.changed() => {
                            if *shutdown_rx.borrow() {
                                ...
                                let _ = writer.write_all(b"QUIT :Captain shutting down\r\n").await;
                                return;
                            }
                        }
                    }
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies all.
    #[test]
    fn test_parse_privmsg_rbac_empty_denies() {
        let line = IrcLine {
            prefix: Some("alice!alice@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#captain".to_string()],
            trailing: Some("hello".to_string()),
        };
        let msg = parse_privmsg(&line, "captain-bot", &[]);
        assert!(msg.is_none(), "empty allowed_users must deny");
    }

    /// Explicit list filters by nickname.
    #[test]
    fn test_parse_privmsg_rbac_explicit_list() {
        let line = IrcLine {
            prefix: Some("alice!alice@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#captain".to_string()],
        ...
        assert!(parse_privmsg(&line, "captain-bot", &allowed).is_some());

        let mallory = IrcLine {
            prefix: Some("mallory!m@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#captain".to_string()],
            trailing: Some("drop".to_string()),
        };
        assert!(parse_privmsg(&mallory, "captain-bot", &allowed).is_none());
    }
    ...
    fn test_parse_irc_line_privmsg() {
        let line = parse_irc_line(":alice!alice@host PRIVMSG #captain :Hello everyone!").unwrap();
        assert_eq!(line.prefix.as_deref(), Some("alice!alice@host"));
        assert_eq!(line.command, "PRIVMSG");
        assert_eq!(line.params, vec!["#captain"]);
    ...
    fn test_parse_irc_line_no_trailing() {
        let line = parse_irc_line(":alice!alice@host JOIN #captain").unwrap();
    ...
    fn test_parse_privmsg_channel() {
        ...
        let msg = parse_privmsg(&line, "captain-bot", &["*".to_string()]).unwrap();
        assert_eq!(msg.channel, ChannelType::Custom("irc".to_string()));
        assert_eq!(msg.sender.display_name, "alice");
        assert_eq!(msg.sender.platform_id, "#captain");
        assert!(msg.is_group);
    ...
    fn test_parse_privmsg_dm() {
        let line = IrcLine {
            ...
            command: "PRIVMSG".to_string(),
            params: vec!["captain-bot".to_string()],
    ...
    fn test_parse_privmsg_skips_self() {
        let line = IrcLine {
            prefix: Some("captain-bot!bot@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#captain".to_string()],
            trailing: Some("My own message".to_string()),
        };

        let msg = parse_privmsg(&line, "captain-bot", &["*".to_string()]);
        assert!(msg.is_none());
    }
    ...
    /// Fix 5 — each successive 433 (ERR_NICKNAMEINUSE) must produce a
    /// distinct alternate nick so retries converge instead of resending the
    /// exact same already-rejected nick forever.
    #[test]
    fn alt_nick_for_attempt_always_advances() {
        let base = "captain";
        let candidates: Vec<String> = (1..=MAX_NICK_ATTEMPTS)
            .map(|attempt| alt_nick_for_attempt(base, attempt))
            .collect();

        assert_eq!(
            candidates,
            vec!["captain_", "captain__", "captain1", "captain2", "captain3"]
        );

        // No two attempts (within the retry budget) ever produce the same
        // candidate — this is what actually prevents the infinite loop.
        let mut seen = std::collections::HashSet::new();
        for candidate in &candidates {
            assert!(
                seen.insert(candidate.clone()),
                "attempt produced a repeated nick: {candidate}"
            );
        }
    }
}
