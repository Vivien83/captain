...
use captain_types::config::{AgentBinding, BroadcastConfig, BroadcastStrategy};
use dashmap::DashMap;
use std::sync::Mutex;
use tracing::warn;
...
pub struct AgentRouter {
    /// Default agent per user (keyed by captain_user or platform_id).
...
impl AgentRouter {
    ...
    /// Resolve using only channel default or system default — skips user_default.
    /// Used for forum topics without a topic→agent mapping.
    pub fn resolve_channel_default(&self, channel_type: &ChannelType) -> Option<AgentId> {
        let channel_key = format!("{channel_type:?}");
        if let Some(agent) = self.channel_defaults.get(&channel_key) {
            return Some(*agent);
        }
...
mod tests {
    ...
    fn test_binding_channel_match() {
        ...
        router.load_bindings(&[AgentBinding {
            ...
            match_rule: captain_types::config::BindingMatchRule {
    ...
    fn test_binding_specificity_scores() {
        use captain_types::config::BindingMatchRule;
