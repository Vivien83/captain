...
pub struct NostrAdapter {
    ...
    relays: Vec<String>,
    /// Nostr pubkeys allowed to address the bot (empty = deny all,
...
impl NostrAdapter {
    ...
    /// * `allowed_users` - Pubkeys authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(private_key: String, relays: Vec<String>, allowed_users: Vec<String>) -> Self {
...
impl ChannelAdapter for NostrAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        info!(
            ...
            captain_types::truncate_str(&pubkey, 16)
        );
                    ...
                    let should_reconnect = loop {
                        let msg = tokio::select! {
                            _ = relay_shutdown_rx.changed() => {
                                ...
                                // Send CLOSE
                                let close_msg = serde_json::json!(["CLOSE", "captain-sub"]);
                                let _ = write.send(
                        ...
                        if !crate::rbac::is_authorized(&allowed_users, &sender_pubkey) {
                            debug!(
                                "Nostr: ignoring event from unauthorised pubkey {sender_pubkey} (allowed_users gate)"
                            );
                            continue;
                        }
                        ...
                        let channel_msg = ChannelMessage {
                            ...
                            sender: ChannelUser {
                                ...
                                display_name: format!(
                                    "{}...",
                                    captain_types::truncate_str(&sender_pubkey, 8)
                                ),
                                captain_user: None,
                            },
                            content: msg_content,
...
mod tests {
    ...
    fn test_nostr_derive_pubkey() {
        let adapter = NostrAdapter::new("deadbeef".repeat(8), vec![], vec!["*".to_string()]);
    ...
    fn test_nostr_build_subscription() {
        let adapter = NostrAdapter::new("abc123".to_string(), vec![], vec!["*".to_string()]);
        let pubkey = adapter.derive_pubkey();
        let sub = adapter.build_subscription(&pubkey);
        assert!(sub.contains("REQ"));
        assert!(sub.contains("captain-sub"));
    ...
    /// B.8 — empty allowed_users denies every pubkey.
    #[test]
    fn test_nostr_rbac_empty_denies() {
        let adapter = NostrAdapter::new("abc".to_string(), vec![], vec![]);
    ...
    /// B.8 — explicit list authorises only listed pubkeys.
    #[test]
    fn test_nostr_rbac_explicit_list() {
        let adapter = NostrAdapter::new("abc".to_string(), vec![], vec!["alicepubkey".to_string()]);
        assert!(crate::rbac::is_authorized(
            &adapter.allowed_users,
            "alicepubkey"
        ));
