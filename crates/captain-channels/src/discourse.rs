...
pub struct DiscourseAdapter {
    ...
    /// Discourse usernames allowed to address the bot (empty = deny all,
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for post in posts.iter().rev() {
                    ...
                    if !crate::rbac::is_authorized(&allowed_users, username) {
                        debug!(
                            "Discourse: ignoring post from unauthorised user {username} (allowed_users gate)"
                        );
                        continue;
                    }
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies every poster.
    #[test]
    fn test_discourse_rbac_empty_denies() {
        let adapter = DiscourseAdapter::new(
            "https://forum.example.com".to_string(),
            "key".to_string(),
            "bot".to_string(),
    ...
    /// B.8 — explicit list authorises only listed posters.
    #[test]
    fn test_discourse_rbac_explicit_list() {
        let adapter = DiscourseAdapter::new(
            "https://forum.example.com".to_string(),
            "key".to_string(),
            "bot".to_string(),
