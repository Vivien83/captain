...
pub struct LineAdapter {
    ...
    webhook_port: u16,
    /// LINE user IDs allowed to address the bot (empty = deny all, ["*"] = allow all).
...
impl LineAdapter {
    ...
    /// * `allowed_users` - LINE user IDs allowed to address the bot (B.8).
    pub fn new(
...
fn parse_line_event(event: &serde_json::Value, allowed_users: &[String]) -> Option<ChannelMessage> {
    ...
    if !crate::rbac::is_authorized(allowed_users, &user_id) {
        debug!("LINE: ignoring event from unauthorised user {user_id} (allowed_users gate)");
        return None;
    }
                    ...
                    move |headers: axum::http::HeaderMap,
                        ...
                        async move {
                            ...
                            if let Some(events) = body.0["events"].as_array() {
                                for event in events {
                                    if let Some(msg) = parse_line_event(event, &users) {
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies even on a valid signed event.
    #[test]
    fn test_parse_line_event_rbac_empty_denies() {
    ...
    fn test_parse_line_event_text_message() {
        ...
        let msg = parse_line_event(&event, &["*".to_string()]).unwrap();
    ...
    fn test_parse_line_event_non_message() {
        ...
        assert!(parse_line_event(&event, &["*".to_string()]).is_none());
    }
