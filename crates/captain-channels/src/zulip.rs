...
pub struct ZulipAdapter {
    ...
    streams: Vec<String>,
    /// Zulip sender emails allowed to address the bot (empty = deny all,
...
impl ZulipAdapter {
    ...
    /// * `allowed_users` - Sender emails authorised to address the bot
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for event in events {
                    ...
                    if !crate::rbac::is_authorized(&allowed_users, sender_email) {
                        debug!(
                            "Zulip: ignoring message from unauthorised sender {sender_email} (allowed_users gate)"
                        );
                        continue;
                    }
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies every sender via the rbac primitive.
    #[test]
    fn test_zulip_rbac_empty_denies() {
        let adapter = ZulipAdapter::new(
            "https://zulip.example.com".to_string(),
            "bot@example.com".to_string(),
            "key".to_string(),
    ...
    /// B.8 — explicit list authorises only listed sender emails.
    #[test]
    fn test_zulip_rbac_explicit_list() {
        let adapter = ZulipAdapter::new(
            "https://zulip.example.com".to_string(),
            "bot@example.com".to_string(),
            "key".to_string(),
