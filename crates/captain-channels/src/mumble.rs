...
/// Decoded Mumble TextMessage protobuf body. Named so the
/// `parse_text_message` return type stays simple for clippy and
/// readers.
struct MumbleTextMessage {
    actor: u32,
    channel_ids: Vec<u32>,
    tree_ids: Vec<u32>,
    session_ids: Vec<u32>,
...
impl MumbleAdapter {
    ...
    fn parse_text_message(payload: &[u8], allowed_users: &[String]) -> Option<MumbleTextMessage> {
        let mut actor: u32 = 0;
        ...
        let sender_id = format!("session-{actor}");
        if !crate::rbac::is_authorized(allowed_users, &sender_id) {
            debug!(
                "mumble: ignoring message from unauthorised session_id {sender_id} (allowed_users gate)"
            );
            return None;
        }

        Some(MumbleTextMessage {
            actor,
            channel_ids,
            tree_ids,
            session_ids,
                        ...
                        match result {
                            Ok(_) => {
                                ...
                                if msg_type == MSG_TYPE_TEXT_MESSAGE {
                                    if let Some(parsed) =
                                        Self::parse_text_message(&payload, &allowed_users)
                                    {
                                        let actor = parsed.actor;
                                        let _ch_ids = parsed.channel_ids;
                                        let _tree_ids = parsed.tree_ids;
                                        let _session_ids = parsed.session_ids;
...
mod tests {
    ...
    fn test_mumble_text_message_roundtrip() {
        ...
        let parsed = MumbleAdapter::parse_text_message(&payload, &["*".to_string()]).unwrap();
        // actor is not set (field 1 omitted) — build only sets channel + message
        assert_eq!(parsed.actor, 0);
        assert_eq!(parsed.channel_ids, vec![42]);
        assert_eq!(parsed.message, "Hello Mumble!");
    }

    #[test]
    fn test_mumble_rbac_empty_denies() {
        let mut payload = Vec::new();
        payload.push(0x08);
        MumbleAdapter::encode_varint(42, &mut payload);
        payload.extend(MumbleAdapter::build_text_message_packet(
            42,
            "Hello Mumble!",
        ));

        assert!(MumbleAdapter::parse_text_message(&payload, &[]).is_none());
    }
