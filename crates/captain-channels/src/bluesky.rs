...
pub struct BlueskyAdapter {
    ...
    /// Bluesky handles allowed to address the bot (empty = deny all,
    /// `["*"]` = allow all). Match is exact against `author.handle`
    /// (e.g. `"alice.bsky.social"`), not the opaque DID.
...
impl BlueskyAdapter {
    ...
    /// * `allowed_users` - Bluesky handles authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(identifier: String, app_password: String, allowed_users: Vec<String>) -> Self {
...
fn parse_bluesky_notification(
    ...
    // Match against the public-facing handle (e.g. "alice.bsky.social"),
    // not the opaque DID — operators write handles in their config.
    let author_handle = author["handle"].as_str().unwrap_or("");
    if !crate::rbac::is_authorized(allowed_users, author_handle) {
        debug!(
            "Bluesky: ignoring notification from unauthorised handle {author_handle} (allowed_users gate)"
        );
        return None;
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for notif in notifications {
                    ...
                    if let Some(msg) = parse_bluesky_notification(notif, &own_did, &allowed_users) {
...
mod tests {
    ...
    fn test_parse_bluesky_notification_mention() {
        ...
        let msg = parse_bluesky_notification(&notif, "did:plc:bot", &["*".to_string()]).unwrap();
    ...
    fn test_parse_bluesky_notification_skips_own() {
        ...
        assert!(parse_bluesky_notification(&notif, "did:plc:bot", &["*".to_string()]).is_none());
    }
    ...
    /// B.8 — empty allowed_users denies every handle.
    #[test]
    fn test_parse_bluesky_notification_rbac_empty_denies() {
        let notif = serde_json::json!({
            "uri": "at://did:plc:sender/app.bsky.feed.post/abc",
            "cid": "...",
            "author": {
                "did": "did:plc:sender",
                "handle": "alice.bsky.social",
    ...
    /// B.8 — explicit list filters by handle.
    #[test]
    fn test_parse_bluesky_notification_rbac_explicit_list() {
        let notif_alice = serde_json::json!({
            "uri": "at://did:plc:1/app.bsky.feed.post/a",
            "cid": "...",
            "author": {
                "did": "did:plc:1",
                "handle": "alice.bsky.social",
        ...
        let notif_mallory = serde_json::json!({
            "uri": "at://did:plc:2/app.bsky.feed.post/b",
            "cid": "...",
            "author": {
                "did": "did:plc:2",
            ...
            "reason": "mention",
            "record": { "text": "@bot drop" },
            "indexedAt": "2024-01-01T00:00:00.000Z"
        });
        let allowed = vec!["alice.bsky.social".to_string()];
        assert!(parse_bluesky_notification(&notif_alice, "did:plc:bot", &allowed).is_some());
        assert!(parse_bluesky_notification(&notif_mallory, "did:plc:bot", &allowed).is_none());
    }
