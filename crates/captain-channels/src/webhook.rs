...
/// Generic HTTP webhook channel adapter.
///
/// The most flexible adapter in the Captain channel suite. Any system that
...
pub struct WebhookAdapter {
    ...
    /// Sender IDs allowed to address the bot (empty = deny all,
...
impl WebhookAdapter {
    ...
    /// * `allowed_users` - Sender IDs authorised to address the bot
                    ...
                    move |headers: axum::http::HeaderMap, body: axum::body::Bytes| {
                        ...
                        async move {
                            ...
                            )) = WebhookAdapter::parse_webhook_body(&json_body)
                            {
                                ...
                                if !crate::rbac::is_authorized(&allowed_users, &sender_id) {
                                    debug!(
                                        "Webhook: ignoring body from unauthorised sender {sender_id} (allowed_users gate)"
                                    );
...
mod tests {
    ...
    fn test_webhook_no_callback() {
        let adapter = WebhookAdapter::new("secret".to_string(), 9000, None, vec!["*".to_string()]);
        assert!(!adapter.has_callback());
    }

    /// B.8 — empty allowed_users denies every sender.
    #[test]
    fn test_webhook_rbac_empty_denies() {
        let adapter = WebhookAdapter::new("secret".to_string(), 9000, None, vec![]);
        assert!(adapter.allowed_users.is_empty());
        assert!(!crate::rbac::is_authorized(&adapter.allowed_users, "alice"));
    }

    /// B.8 — explicit list authorises only listed senders.
    #[test]
    fn test_webhook_rbac_explicit_list() {
        let adapter =
            WebhookAdapter::new("secret".to_string(), 9000, None, vec!["alice".to_string()]);
        assert!(crate::rbac::is_authorized(&adapter.allowed_users, "alice"));
