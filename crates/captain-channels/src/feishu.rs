...
pub struct FeishuAdapter {
    ...
    /// Feishu/Lark `open_id` values allowed to address the bot
...
impl FeishuAdapter {
    ...
    ///
    /// `allowed_users` enforces the B.8 deny-by-default contract: empty list
    /// rejects every sender; `["*"]` opts back into permissive intake.
    ...
    pub fn with_config(
        ...
        adapter.allowed_users = allowed_users;
        adapter
    }
...
fn parse_event(
    ...
    let sender_open_id = sender
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("");
    if !crate::rbac::is_authorized(allowed_users, sender_open_id) {
        debug!(
            "Feishu/Lark: ignoring message from unauthorised open_id {sender_open_id} (allowed_users gate)"
        );
        return None;
    }
                            ...
                            if schema == Some("2.0") {
                                ...
                                if event_type == "message" {
                                    ...
                                    if !text.is_empty() {
                                        ...
                                        // B.8 — empty allowed_users denies all on V1 path too.
                                        if !crate::rbac::is_authorized(&allowed_users, &open_id) {
                                            debug!(
                                                "{region_label}: V1 ignoring message from unauthorised open_id {open_id} (allowed_users gate)"
                                            );
                                            return (
                                                axum::http::StatusCode::OK,
                                                axum::Json(serde_json::json!({"code": 0})),
...
mod tests {
    ...
    #[test]
    fn test_parse_event_rbac_empty_denies() {
    ...
    /// B.8 — explicit list authorises only listed open_ids.
    #[test]
    fn test_parse_event_rbac_explicit_list() {
        let mk = |open_id: &str| {
            serde_json::json!({
                "schema": "2.0",
                "header": {
                    "event_id": format!("evt-{open_id}"),
                    "event_type": "im.message.receive_v1",
                },
                "event": {
                    "sender": {
        ...
        let allowed = vec!["ou_alice".to_string()];
        assert!(parse_event(&mk("ou_alice"), &[], "feishu", &allowed).is_some());
        assert!(parse_event(&mk("ou_mallory"), &[], "feishu", &allowed).is_none());
    }
    ...
    fn test_parse_event_v2_text() {
        ...
        let msg = parse_event(&event, &[], "feishu", &["*".to_string()]).unwrap();
        ...
        let msg = parse_event(&event, &[], "lark", &["*".to_string()]).unwrap();
    ...
    fn test_parse_event_group_filters() {
        ...
        assert!(parse_event(&event, &[], "feishu", &["*".to_string()]).is_none());
    }
