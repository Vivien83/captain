...
pub struct DingTalkAdapter {
    ...
    webhook_port: u16,
    /// DingTalk sender IDs allowed to address the bot (empty = deny all,
                        ...
                        async move {
                                ...
                                DingTalkAdapter::parse_callback(&body)
                            {
                                ...
                                if !crate::rbac::is_authorized(&allowed_users, &sender_id) {
                                    debug!(
                                        "DingTalk: ignoring callback from unauthorised sender {sender_id} (allowed_users gate)"
                                    );
...
mod tests {
    ...
    #[test]
    fn test_dingtalk_rbac_empty_denies() {
        let adapter = DingTalkAdapter::new("tok".to_string(), "secret".to_string(), 8080, vec![]);
    ...
    /// B.8 — explicit list authorises only listed senderIds.
    #[test]
    fn test_dingtalk_rbac_explicit_list() {
        let adapter = DingTalkAdapter::new(
            "tok".to_string(),
            "secret".to_string(),
            8080,
