...
pub struct PumbleAdapter {
    ...
    webhook_port: u16,
    /// Pumble user IDs allowed to address the bot (empty = deny all,
...
impl PumbleAdapter {
    ...
    /// * `allowed_users` - Pumble user IDs authorised to address the bot
...
fn parse_pumble_event(
    ...
    if !crate::rbac::is_authorized(allowed_users, user_id) {
        debug!("Pumble: ignoring event from unauthorised user {user_id} (allowed_users gate)");
        return None;
    }
                ...
                axum::routing::post({
                    ...
                    move |body: axum::extract::Json<serde_json::Value>| {
                        ...
                        async move {
                            ...
                            if let Some(msg) = parse_pumble_event(&body, &bot_id, &allowed_users) {
...
mod tests {
    ...
    fn test_pumble_adapter_creation() {
        let adapter = PumbleAdapter::new("test-bot-token".to_string(), 8080, vec!["*".to_string()]);
    ...
    fn test_pumble_webhook_port() {
        let adapter = PumbleAdapter::new("token".to_string(), 9999, vec!["*".to_string()]);
        assert_eq!(adapter.webhook_port, 9999);
    }

    /// B.8 — empty allowed_users denies every user.
    #[test]
    fn test_parse_pumble_event_rbac_empty_denies() {
        ...
        assert!(parse_pumble_event(&event, "BOT001", &[]).is_none());
    }

    /// B.8 — explicit list authorises only listed users.
    #[test]
    fn test_parse_pumble_event_rbac_explicit_list() {
        ...
        assert!(parse_pumble_event(&alice, "BOT001", &allowed).is_some());
        assert!(parse_pumble_event(&mallory, "BOT001", &allowed).is_none());
    }
    ...
    fn test_parse_pumble_event_message() {
        ...
        let msg = parse_pumble_event(&event, "BOT001", &["*".to_string()]).unwrap();
    ...
    fn test_parse_pumble_event_skip_bot() {
        ...
        let msg = parse_pumble_event(&event, "BOT001", &["*".to_string()]);
        assert!(msg.is_none());
    }
