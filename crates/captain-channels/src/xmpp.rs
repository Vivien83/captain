...
pub struct XmppAdapter {
    ...
    rooms: Vec<String>,
    /// JIDs allowed to address the bot (empty = deny all, `["*"]` = allow all).
    /// B.8 contract: when the real `start()` implementation lands (with the
    /// `tokio-xmpp` dependency), the inbound stanza handler MUST consult this
    /// list via `crate::rbac::is_authorized(&self.allowed_users, from_jid)`
    /// before forwarding messages — exactly like the other 23 channel adapters.
    #[allow(dead_code)]
...
impl XmppAdapter {
    ...
    /// * `allowed_users` - JIDs authorised to address the bot (B.8 contract:
    ///   empty list denies all; `["*"]` opts back into permissive). Stored
    ///   on the adapter so the future stream handler can enforce it.
    pub fn new(
        jid: String,
        password: String,
        server: String,
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies every JID. Stored on the adapter so
    /// the future tokio-xmpp stream handler can enforce the contract.
    #[test]
    fn test_xmpp_rbac_empty_denies() {
        let adapter = XmppAdapter::new(
            "bot@example.com".to_string(),
            "pass".to_string(),
            "xmpp.example.com".to_string(),
    ...
    /// B.8 — explicit list authorises only listed JIDs.
    #[test]
    fn test_xmpp_rbac_explicit_list() {
        let adapter = XmppAdapter::new(
            "bot@example.com".to_string(),
            "pass".to_string(),
            "xmpp.example.com".to_string(),
