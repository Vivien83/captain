//! Slack Socket Mode adapter for the Captain channel bridge.
//!
...
async fn parse_slack_event(
    ...
    // Without this gate, a Slack workspace member outside the allowlist could
    // execute /clear, /compact, /stop and other gated commands.
    if !crate::rbac::is_authorized(allowed_users, user_id) {
        debug!("Slack: ignoring message from unauthorised user {user_id} (allowed_users gate)");
        return None;
    }
...
mod tests {
    ...
    /// Helper: `["*"]` opt-in to allow every Slack user, used by tests that
    /// want to exercise non-RBAC paths without re-typing the wildcard.
    fn any_user() -> Vec<String> {
        vec!["*".to_string()]
    }
    ...
    /// B.8 — empty allowed_users denies every sender, even authorised channels.
    /// Without this gate, anyone in the workspace could trigger gated commands.
    #[tokio::test]
    async fn test_parse_slack_event_rbac_empty_denies() {
        let bot_id = Arc::new(RwLock::new(None));
        let event = serde_json::json!({
            "type": "message",
            "user": "U456",
        ...
        let msg =
            parse_slack_event(&event, &bot_id, &[], &[], &Arc::new(DashMap::new()), true).await;
        assert!(msg.is_none(), "empty allowed_users must deny");
    }

    /// Explicit allowlist: listed users pass, unlisted users fail.
    #[tokio::test]
    async fn test_parse_slack_event_rbac_explicit_list() {
        let bot_id = Arc::new(RwLock::new(None));
        let event = serde_json::json!({
            "type": "message",
            "user": "U456",
        ...
        let allowed = vec!["U456".to_string(), "UALICE".to_string()];
        let msg = parse_slack_event(
            &event,
            &bot_id,
            &allowed,
        ...
        .await;
        assert!(msg.is_some(), "listed user must pass");
        ...
        .await;
        assert!(msg.is_none(), "unlisted user must fail");
    }
