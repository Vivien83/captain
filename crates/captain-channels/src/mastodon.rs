...
/// After falling back to REST polling, retry the SSE streaming endpoint
/// every this many polling cycles (at `SSE_RECONNECT_DELAY_SECS` each) in
/// case the instance's streaming endpoint has recovered.
const STREAMING_RETRY_INTERVAL_POLLS: u32 = 60;

/// Whether the polling loop should attempt SSE streaming again, given how
/// many REST polling cycles have elapsed since streaming last failed.
fn should_retry_streaming(polls_since_retry: u32) -> bool {
    polls_since_retry >= STREAMING_RETRY_INTERVAL_POLLS
}
...
pub struct MastodonAdapter {
    ...
    /// Mastodon `acct` handles allowed to address the bot (empty = deny all,
    /// `["*"]` = allow all). Match is exact against `account.acct` (e.g.
    /// `"alice"` for local accounts, `"alice@mastodon.social"` for remote).
...
impl MastodonAdapter {
    ...
    /// * `allowed_users` - Mastodon `acct` handles authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(instance_url: String, access_token: String, allowed_users: Vec<String>) -> Self {
...
fn parse_mastodon_notification(
    ...
    // B.8 — empty allowed_users denies all. ["*"] opts back into permissive.
    // Match against the public-facing `acct` handle (e.g. "alice" or
    // "alice@mastodon.social"), not the opaque internal account id.
    let acct_handle = account["acct"].as_str().unwrap_or("");
    if !crate::rbac::is_authorized(allowed_users, acct_handle) {
        debug!(
            "Mastodon: ignoring mention from unauthorised acct {acct_handle} (allowed_users gate)"
        );
        return None;
    }
...
impl ChannelAdapter for MastodonAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        tokio::spawn(async move {
            ...
            // Once SSE fails, `use_streaming` used to stay false forever even if the
            // instance's streaming endpoint later recovers. Retry streaming every
            // STREAMING_RETRY_INTERVAL_POLLS polling cycles instead of falling back
            // to REST polling permanently.
            let mut polls_since_streaming_retry: u32 = 0;
            ...
            loop {
                ...
                polls_since_streaming_retry += 1;
                if should_retry_streaming(polls_since_streaming_retry) {
                    info!("Mastodon: retrying SSE streaming after REST polling fallback");
                    use_streaming = true;
                    polls_since_streaming_retry = 0;
                    continue;
                }
                ...
                for notif in &notifications {
                    if let Some(msg) =
                        parse_mastodon_notification(notif, &own_account_id, &allowed_users)
                    {
...
mod tests {
    ...
    /// Fix — once SSE streaming fails and the loop falls back to REST
    /// polling, it must periodically retry streaming instead of staying on
    /// polling forever even if the instance's streaming endpoint recovers.
    #[test]
    fn should_retry_streaming_fires_after_interval_not_before() {
        for polls in 0..STREAMING_RETRY_INTERVAL_POLLS {
            assert!(
                !should_retry_streaming(polls),
                "must not retry streaming before the interval elapses (polls={polls})"
            );
        }
        assert!(should_retry_streaming(STREAMING_RETRY_INTERVAL_POLLS));
        assert!(should_retry_streaming(STREAMING_RETRY_INTERVAL_POLLS + 1));
    }
    ...
    fn test_mastodon_custom_instance() {
        let adapter = MastodonAdapter::new(
            "https://infosec.exchange".to_string(),
            "tok".to_string(),
            vec!["*".to_string()],
        );
    ...
    fn test_parse_mastodon_notification_mention() {
        ...
        let msg = parse_mastodon_notification(&notif, "acct-999", &["*".to_string()]).unwrap();
    ...
    fn test_parse_mastodon_notification_non_mention() {
        ...
        assert!(parse_mastodon_notification(&notif, "acct-999", &["*".to_string()]).is_none());
    }
    ...
    /// B.8 — empty allowed_users denies every acct.
    #[test]
    fn test_parse_mastodon_notification_rbac_empty_denies() {
        let notif = serde_json::json!({
            "id": "notif-1",
            "type": "mention",
            "account": {
    ...
    /// B.8 — explicit acct list filters correctly.
    #[test]
    fn test_parse_mastodon_notification_rbac_explicit_list() {
        let notif_alice = serde_json::json!({
            "id": "notif-1",
            "type": "mention",
            "account": {
                "id": "acct-1",
        ...
        let notif_mallory = serde_json::json!({
            "id": "notif-2",
            "type": "mention",
            "account": {
                "id": "acct-2",
            ...
            "status": {
                "id": "status-2",
                "content": "<p>@bot drop</p>",
                "visibility": "public",
                "in_reply_to_id": null
            }
        });
        let allowed = vec!["alice".to_string()];
        assert!(parse_mastodon_notification(&notif_alice, "acct-999", &allowed).is_some());
        assert!(parse_mastodon_notification(&notif_mallory, "acct-999", &allowed).is_none());
    }
