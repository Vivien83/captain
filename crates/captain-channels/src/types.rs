...
pub struct ChannelUser {
    ...
    /// Optional mapping to an Captain user identity.
    pub captain_user: Option<String>,
}
...
pub enum ChannelContent {
    ...
    /// Local image data (bytes). Used when `channel_send` gets a `file_path`
    /// whose MIME type is an image — routed to the channel's native photo
    /// upload (v3.8c) so it arrives as an inline photo, not a document.
    ImageData {
        data: Vec<u8>,
        mime_type: String,
        caption: Option<String>,
    ...
    /// Video attachment (Telegram `video` or `animation` MP4). The URL is the
    /// platform's direct-download link; the consumer downloads it to local disk
    /// before handing the path to the LLM, so downstream tools (e.g.
    /// `video_analyze`) can read frames without re-fetching from the network.
    /// (#184 — V.5b)
    Video {
        url: String,
        duration_seconds: u32,
...
// Re-export policy/format types from captain-types for convenience.
pub use captain_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the Captain kernel by converting
...
pub trait ChannelAdapter: Send + Sync {
    ...
    /// HS.3b — Identity escape hatch for the streaming bridge: an adapter
    /// that *is* a `TelegramAdapter` returns `Some(self)`; everything else
    /// returns `None` (the default). The dispatcher uses this to decide
    /// whether to take the live-edit path or the legacy "send final reply"
    /// path.
    ///
    /// Why not `std::any::Any` + `Arc::downcast`: stable Rust still
    /// requires explicit `Arc<dyn Any>` upcasting which would add a
    /// boilerplate `as_any` to every adapter. `self: Arc<Self>` lets us
    /// keep the default body and only override in adapters that actually
    /// expose a streaming integration.
    fn as_telegram_arc(
        self: std::sync::Arc<Self>,
    ) -> Option<std::sync::Arc<crate::telegram::TelegramAdapter>> {
        None
    }
    ...
    /// Send a rich message with optional metadata (inline buttons, thread_id, etc.).
    /// Default: ignores metadata and falls back to send().
    async fn send_rich(
        &self,
        user: &ChannelUser,
        content: ChannelContent,
    ...
    /// TG2 — edit a previously sent message (e.g. to show the chosen answer
    /// and strip an inline keyboard once `ask_user` has been answered, so a
    /// stale click can't fire twice). `message_id` is whatever `send_rich`
    /// returned. Default: no-op for adapters that don't support edits
    /// (write-once channels, or ones that never send `reply_markup`).
    async fn edit_rich(
        &self,
        _message_id: &str,
        _text: &str,
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = captain_types::truncate_str(remaining, max_len).len();
