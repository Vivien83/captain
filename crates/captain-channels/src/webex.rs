...
pub struct WebexAdapter {
    ...
    /// Webex personIds allowed to address the bot (empty = deny all,
...
impl WebexAdapter {
    ...
    /// * `allowed_users` - personIds authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(bot_token: String, allowed_rooms: Vec<String>, allowed_users: Vec<String>) -> Self {
            ...
            loop {
                ...
                let should_reconnect = loop {
                    ...
                    if !crate::rbac::is_authorized(&allowed_users, &sender_id) {
                        debug!(
                            "Webex: ignoring message from unauthorised personId {sender_id} (allowed_users gate)"
                        );
                        continue;
                    }
...
mod tests {
    ...
    fn test_webex_allowed_rooms() {
        ...
        let open = WebexAdapter::new("tok".to_string(), vec![], vec!["*".to_string()]);
    ...
    /// B.8 — empty allowed_users denies every personId.
    #[test]
    fn test_webex_rbac_empty_denies() {
        let adapter = WebexAdapter::new("tok".to_string(), vec![], vec![]);
    ...
    /// B.8 — explicit list authorises only listed personIds.
    #[test]
    fn test_webex_rbac_explicit_list() {
        let adapter = WebexAdapter::new(
            "tok".to_string(),
            vec![],
            vec!["personId-alice".to_string()],
        );
        assert!(crate::rbac::is_authorized(
            &adapter.allowed_users,
            "personId-alice"
