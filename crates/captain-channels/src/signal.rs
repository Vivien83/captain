...
impl SignalAdapter {
    ...
    fn is_allowed(&self, phone: &str) -> bool {
        rbac::is_authorized(&self.allowed_users, phone)
    }
}
        ...
        tokio::spawn(async move {
            loop {
                ...
                for msg in messages {
                    ...
                    if !rbac::is_authorized(&allowed_users, &source) {
                        continue;
                    }
                    ...
                    if tx.send(channel_msg).await.is_err() {
                        // Receiver dropped (adapter shutting down / bridge
                        // gone) — stop the task entirely, not just this
                        // inner `for` loop, matching every other adapter.
                        return;
                    }
                }
            }
...
mod tests {
    ...
    fn test_signal_allowed_check() {
        ...
        let closed = SignalAdapter::new(
            "http://localhost:8080".to_string(),
            "+1234567890".to_string(),
            vec![],
        );
        assert!(!closed.is_allowed("+anything"));

        let open = SignalAdapter::new(
            "http://localhost:8080".to_string(),
            "+1234567890".to_string(),
            vec!["*".to_string()],
        );
    ...
    /// Fix 4 — when the mpsc receiver is dropped, `tx.send(...).is_err()`
    /// must stop the whole polling task (`return`), not just the inner
    /// `for msg in messages` loop (`break`). With the old `break`, the task
    /// keeps polling forever even though nothing can ever consume a message
    /// again. We assert this by dropping the returned stream immediately and
    /// checking the mock server receives exactly one poll — a second poll
    /// would mean the outer `loop {}` survived the failed send.
    #[tokio::test]
    async fn dropped_receiver_stops_the_polling_task_entirely() {
        ...
        let stream = adapter.start().await.expect("start must succeed");
        // Drop the receiver right away so the next `tx.send` inside the
        // spawned polling task fails.
        drop(stream);

        // First poll fires at ~POLL_INTERVAL (2s); a second poll would only
        // happen if the task incorrectly kept looping after the failed send.
        tokio::time::sleep(POLL_INTERVAL + Duration::from_secs(3)).await;

        let requests = server
            .received_requests()
            .await
            .expect("mock server must track requests");
        assert_eq!(
            requests.len(),
            1,
            "polling task must stop after the first failed send instead of looping forever"
        );
    }
}
