...
pub struct WeComAdapter {
    ...
    webhook_port: u16,
    /// WeCom user IDs allowed to address the bot (empty = deny all, ["*"] = allow all).
...
impl WeComAdapter {
    ...
    pub fn with_verification(
        ...
        let mut adapter = Self::new(corp_id, agent_id, secret, webhook_port, allowed_users);
                ...
                axum::routing::get({
                    ...
                    move |axum::extract::Query(params): axum::extract::Query<std::collections::HashMap<String, String>>, body: String| {
                        ...
                        async move {
                            ...
                            // ["*"] opts back into permissive intake.
                            if !crate::rbac::is_authorized(&users, &user_id) {
                                debug!(
                                    "WeCom: ignoring callback from unauthorised user {user_id} (allowed_users gate)"
                                );
...
mod tests {
    ...
    fn test_decode_wecom_payload() {
        ...
        assert!(plain.ends_with("-wecom-check"));
    }
