...
pub struct LinkedInAdapter {
    ...
    /// LinkedIn member URNs allowed to address the bot (empty = deny all,
...
impl LinkedInAdapter {
    ...
    /// * `allowed_users` - Member URNs authorised to address the bot
    ///   (B.8 contract: empty list denies all; `["*"]` opts back into permissive).
    pub fn new(access_token: String, organization_id: String, allowed_users: Vec<String>) -> Self {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for element in &messages {
                    ...
                    if !crate::rbac::is_authorized(&allowed_users, &sender_urn) {
                        debug!(
                            "LinkedIn: ignoring message from unauthorised sender {sender_urn} (allowed_users gate)"
                        );
                        continue;
                    }
...
mod tests {
    ...
    fn test_linkedin_organization_id_normalization() {
        ...
        let adapter2 = LinkedInAdapter::new(
            "tok".to_string(),
            "urn:li:organization:67890".to_string(),
            vec!["*".to_string()],
        );
    ...
    /// B.8 — empty allowed_users denies every member.
    #[test]
    fn test_linkedin_rbac_empty_denies() {
        let adapter = LinkedInAdapter::new("tok".to_string(), "12345".to_string(), vec![]);
    ...
    /// B.8 — explicit list authorises only listed member URNs.
    #[test]
    fn test_linkedin_rbac_explicit_list() {
        let adapter = LinkedInAdapter::new(
            "tok".to_string(),
            "12345".to_string(),
            vec!["urn:li:person:alice".to_string()],
        );
        assert!(crate::rbac::is_authorized(
            &adapter.allowed_users,
            "urn:li:person:alice"
