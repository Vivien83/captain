...
pub struct DingTalkStreamAdapter {
    ...
    app_secret: String,
    robot_code: String,
    /// DingTalk staff/sender IDs allowed to address the bot (empty = deny all,
                        ...
                        msg = source.next() => {
                            match msg {
                                ...
                                Some(Ok(Message::Text(text))) => {
                                    handle_frame(&text, &mut sink, &tx, &allowed_users).await;
                                }
                ...
                if cb.msg_type == "text" {
                    if let Some(ref tc) = cb.text {
                        ...
                        if !trimmed.is_empty() {
                            ...
                            if !crate::rbac::is_authorized(allowed_users, &uid) {
                                debug!(
                                    "DingTalk Stream: ignoring frame from unauthorised sender {uid} (allowed_users gate)"
                                );
                                return;
                            }
...
mod tests {
    ...
    fn adapter_creation() {
        let a =
            DingTalkStreamAdapter::new("k".into(), "s".into(), "r".into(), vec!["*".to_string()]);
    ...
    /// B.8 — empty allowed_users denies every staff/sender.
    #[test]
    fn rbac_empty_denies() {
        let a = DingTalkStreamAdapter::new("k".into(), "s".into(), "r".into(), vec![]);
        assert!(a.allowed_users.is_empty());
        assert!(!crate::rbac::is_authorized(&a.allowed_users, "staff-alice"));
    }

    /// B.8 — explicit list authorises only listed staff IDs.
    #[test]
    fn rbac_explicit_list() {
        let a = DingTalkStreamAdapter::new(
            "k".into(),
            "s".into(),
            "r".into(),
            vec!["staff-alice".to_string()],
        );
        assert!(crate::rbac::is_authorized(&a.allowed_users, "staff-alice"));
