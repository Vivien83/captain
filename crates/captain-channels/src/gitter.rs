...
impl GitterAdapter {
    ...
    pub fn new(token: String, room_id: String, allowed_users: Vec<String>) -> Self {
    ...
    fn parse_stream_message(
        ...
        if !crate::rbac::is_authorized(allowed_users, &username) {
            debug!(
                "gitter: ignoring message from unauthorised username {username} (allowed_users gate)"
            );
            return None;
        }
                                ...
                                Some(Ok(bytes)) => {
                                    ...
                                    while let Some(newline_pos) = line_buffer.find('\n') {
                                        ...
                                        if let Some((id, text, username, display_name)) =
                                            Self::parse_stream_message(&line, &allowed_users)
                                        {
                                            // Skip own messages
...
mod tests {
    ...
    fn test_gitter_parse_stream_message() {
        ...
        let result = GitterAdapter::parse_stream_message(json, &["*".to_string()]);
    ...
    fn test_gitter_parse_stream_message_missing_fields() {
        let json = r#"{"id":"msg1"}"#;
        assert!(GitterAdapter::parse_stream_message(json, &["*".to_string()]).is_none());
    }
    ...
    fn test_gitter_parse_invalid_json() {
        assert!(GitterAdapter::parse_stream_message("not json", &["*".to_string()]).is_none());
        assert!(GitterAdapter::parse_stream_message("", &["*".to_string()]).is_none());
    }

    #[test]
    fn test_gitter_rbac_empty_denies() {
        let json = r#"{"id":"msg1","text":"Hello","fromUser":{"username":"alice","displayName":"Alice B"}}"#;
