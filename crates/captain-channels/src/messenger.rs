...
fn parse_messenger_entry(
    ...
    for event in messaging {
        ...
        if !crate::rbac::is_authorized(allowed_users, &sender_id) {
            debug!(
                "messenger: ignoring message from unauthorised sender_id {sender_id} (allowed_users gate)"
            );
            continue;
        }
                        ...
                        async move {
                            ...
                            if let Some(entries) = body.0["entry"].as_array() {
                                for entry in entries {
                                    let msgs = parse_messenger_entry(entry, &users);
                                    for msg in msgs {
...
mod tests {
    ...
    fn test_parse_messenger_entry_text_message() {
        ...
        let msgs = parse_messenger_entry(&entry, &["*".to_string()]);
    ...
    #[test]
    fn test_messenger_rbac_empty_denies() {
        let entry = serde_json::json!({
            "id": "page-id",
            "messaging": [
                {
