...
/// Captain channel system. Supports multiple channels simultaneously.
pub struct TwitchAdapter {
    /// SECURITY: OAuth token is zeroized on drop.
    oauth_token: Zeroizing<String>,
    /// Twitch nicks allowed to address the bot (empty = deny all, ["*"] = allow all).
...
impl TwitchAdapter {
    ...
    /// * `allowed_users` - Twitch nicks authorised to address the bot (B.8 contract:
                ...
                let should_reconnect = loop {
                    ...
                    if let Some((sender_nick, channel, message)) = parse_privmsg(line) {
                        ...
                        if !crate::rbac::is_authorized(&allowed_users, &sender_nick) {
                            debug!(
                                "Twitch: ignoring PRIVMSG from unauthorised nick {sender_nick} (allowed_users gate)"
                            );
                            continue;
                        }
...
mod tests {
    ...
    /// B.8 — empty allowed_users must deny every sender (Twitch nicks).
    #[test]
    fn test_twitch_rbac_empty_denies() {
        let adapter = TwitchAdapter::new(
            "abc123".to_string(),
            vec![],
            vec!["captainstream".to_string()],
            "captain_bot".to_string(),
        );
    ...
    /// B.8 — explicit list authorises only the listed nicks.
    #[test]
    fn test_twitch_rbac_explicit_list() {
        let adapter = TwitchAdapter::new(
            "abc123".to_string(),
        ...
        assert!(crate::rbac::is_authorized(&adapter.allowed_users, "bob"));
        assert!(!crate::rbac::is_authorized(&adapter.allowed_users, "carol"));
    }
