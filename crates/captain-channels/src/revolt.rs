...
pub struct RevoltAdapter {
    ...
    ws_url: String,
    /// Revolt user IDs allowed to address the bot (empty = deny all,
    /// `["*"]` = allow all). Match is exact against the message `author` id.
...
impl RevoltAdapter {
    ...
    /// * `allowed_users` - Revolt user IDs authorised to address the bot
    ...
    pub fn with_channels(
        ...
        let mut adapter = Self::new(bot_token, allowed_users);
...
fn parse_revolt_message(
    ...
    if !crate::rbac::is_authorized(allowed_users, author) {
        debug!("Revolt: ignoring message from unauthorised author {author} (allowed_users gate)");
        return None;
    }
...
mod tests {
    ...
    fn test_revolt_adapter_creation() {
        let adapter = RevoltAdapter::new("bot-token-123".to_string(), vec!["*".to_string()]);
    ...
    fn test_revolt_default_urls() {
        let adapter = RevoltAdapter::new("tok".to_string(), vec!["*".to_string()]);
    ...
    /// B.8 — empty allowed_users denies every author.
    #[test]
    fn test_parse_revolt_message_rbac_empty_denies() {
    ...
    /// B.8 — explicit list authorises only listed authors.
    #[test]
    fn test_parse_revolt_message_rbac_explicit_list() {
        let data_alice = serde_json::json!({
            "type": "Message",
            "_id": "msg-a",
            "channel": "ch-1",
            "author": "user-alice",
        ...
        let data_mallory = serde_json::json!({
            "type": "Message",
            "_id": "msg-m",
            "channel": "ch-1",
            "author": "user-mallory",
        ...
        let allowed = vec!["user-alice".to_string()];
        assert!(parse_revolt_message(&data_alice, "bot-id", &allowed, &[]).is_some());
        assert!(parse_revolt_message(&data_mallory, "bot-id", &allowed, &[]).is_none());
    }
    ...
    fn test_revolt_auth_header() {
        let adapter = RevoltAdapter::new("my-revolt-token".to_string(), vec!["*".to_string()]);
    ...
    fn test_parse_revolt_message_basic() {
        ...
        let msg = parse_revolt_message(&data, "bot-id", &["*".to_string()], &[]).unwrap();
    ...
    fn test_parse_revolt_message_skips_bot() {
        ...
        assert!(parse_revolt_message(&data, "bot-id", &["*".to_string()], &[]).is_none());
    }
    ...
    fn test_parse_revolt_message_channel_filter() {
        ...
        assert!(parse_revolt_message(
            &data,
            "bot-id",
            &["*".to_string()],
            &["ch-allowed".to_string()]
        )
        .is_none());
