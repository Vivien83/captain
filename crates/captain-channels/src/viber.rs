...
const DEFAULT_SENDER_NAME: &str = "Captain";
...
impl ViberAdapter {
    ...
    pub fn with_sender(
        ...
        let mut adapter = Self::new(auth_token, webhook_url, webhook_port, allowed_users);
...
fn parse_viber_event(
    ...
    if !crate::rbac::is_authorized(allowed_users, &sender_id) {
        debug!(
            "viber: ignoring message from unauthorised sender_id {sender_id} (allowed_users gate)"
        );
        return None;
    }
            ...
            let app = axum::Router::new().route(
                ...
                axum::routing::post({
                    ...
                    move |body: axum::extract::Json<serde_json::Value>| {
                        ...
                        let tx = Arc::clone(&tx);
                        async move {
                            if let Some(msg) = parse_viber_event(&body.0, &users) {
...
mod tests {
    ...
    fn test_parse_viber_event_text_message() {
        ...
        let msg = parse_viber_event(&event, &["*".to_string()]).unwrap();
    ...
    fn test_parse_viber_event_non_message() {
        ...
        assert!(parse_viber_event(&event, &["*".to_string()]).is_none());
    }
    ...
    #[test]
    fn test_viber_rbac_empty_denies() {
        let event = serde_json::json!({
            "event": "message",
            "message_token": 123_u64,
            "sender": {
