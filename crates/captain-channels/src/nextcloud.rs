...
pub struct NextcloudAdapter {
    ...
    /// Nextcloud actor IDs allowed to address the bot (empty = deny all,
...
impl NextcloudAdapter {
    ...
    /// * `allowed_users` - Actor IDs authorised to address the bot
...
/// Discover joined rooms via the Nextcloud Talk API.
///
/// A transient network error here must not permanently disable the channel:
/// retries indefinitely with the same capped exponential backoff shape used
/// by the per-room polling loop, instead of giving up after the first
/// attempt. Returns `None` only if `shutdown_rx` fires while waiting to
/// retry, so the caller can exit cleanly during shutdown.
async fn discover_rooms_with_retry(
    client: &reqwest::Client,
    server_url: &str,
    token: &str,
    shutdown_rx: &mut watch::Receiver<bool>,
) -> Option<Vec<String>> {
    let url = format!("{server_url}/ocs/v2.php/apps/spreed/api/v4/room?format=json");
    ...
    loop {
            ...
            .await
        {
            ...
            Err(e) => {
                warn!("Nextcloud: failed to list rooms (retrying in {backoff:?}): {e}");
                tokio::select! {
                    _ = shutdown_rx.changed() => {
                        info!("Nextcloud Talk adapter shutting down during room discovery");
                        return None;
                    }
...
impl ChannelAdapter for NextcloudAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        tokio::spawn(async move {
            // Determine rooms to poll. A transient network error here must not
            // permanently disable the channel — retry with backoff instead of
            // returning and leaving the adapter dead.
            let rooms_to_poll = if allowed_rooms.is_empty() {
                match discover_rooms_with_retry(
                    &client,
                    &server_url,
                    token.as_str(),
                    &mut shutdown_rx,
            ...
            loop {
                ...
                for room_token in &rooms_to_poll {
                    ...
                    for msg in messages {
                        ...
                        if !crate::rbac::is_authorized(&allowed_users, actor_id) {
                            debug!(
                                "Nextcloud: ignoring message from unauthorised actor {actor_id} (allowed_users gate)"
                            );
                            continue;
                        }
...
mod tests {
    ...
    /// B.8 — empty allowed_users denies every actor.
    #[test]
    fn test_nextcloud_rbac_empty_denies() {
        let adapter = NextcloudAdapter::new(
            "https://cloud.example.com".to_string(),
            "tok".to_string(),
            vec![],
        ...
        assert!(adapter.allowed_users.is_empty());
        assert!(!crate::rbac::is_authorized(&adapter.allowed_users, "alice"));
    }

    /// B.8 — explicit list authorises only listed actors.
    #[test]
    fn test_nextcloud_rbac_explicit_list() {
        let adapter = NextcloudAdapter::new(
            "https://cloud.example.com".to_string(),
            "tok".to_string(),
            vec![],
        ...
        assert!(crate::rbac::is_authorized(&adapter.allowed_users, "alice"));
        assert!(!crate::rbac::is_authorized(
            &adapter.allowed_users,
            "mallory"
        ));
    }

    /// Fix 2 — a transient network failure on the initial room-listing call
    /// must be retried (with backoff) instead of permanently disabling the
    /// channel. The first attempt hits a closed port (real connection-refused
    /// error), then a server comes up on that same port and the retry must
    /// succeed.
    #[tokio::test]
    async fn discover_rooms_retries_after_transient_network_failure() {
        use wiremock::matchers::{method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};

        let listener = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let addr = listener.local_addr().unwrap();
        drop(listener); // free the port so the first connect fails fast
        ...
        let discover = tokio::spawn({
            let url = url.clone();
            async move { discover_rooms_with_retry(&client, &url, "test-token", &mut shutdown_rx).await }
        });

        // Let the first attempt fail against the closed port and start
        // backing off before the server comes up.
        tokio::time::sleep(Duration::from_millis(300)).await;

        let tcp_listener =
            std::net::TcpListener::bind(addr).expect("port must be free again after drop");
        let server = MockServer::builder().listener(tcp_listener).start().await;
        Mock::given(method("GET"))
            .and(path("/ocs/v2.php/apps/spreed/api/v4/room"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "ocs": { "data": [ { "token": "room-1" } ] }
            })))
            .mount(&server)
            .await;

        let rooms = tokio::time::timeout(Duration::from_secs(10), discover)
            .await
            .expect("discover_rooms_with_retry must eventually succeed")
            .expect("task must not panic");

        assert_eq!(rooms, Some(vec!["room-1".to_string()]));
    }
}
