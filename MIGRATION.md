# Migrating to Captain

This guide covers migrating from OpenClaw (and other frameworks) to Captain. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.captain/`.

### Options

```bash
# Specify a custom source directory
captain migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.captain/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
captain migrate --from langchain   # Coming soon
captain migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (Captain) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.captain/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.captain/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.captain/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.captain/channels_import.toml` | Automated (manual merge) |
...
Channel configurations are exported to a `channels_import.toml` file. You must
manually merge only the active core sections you intend to run into
`~/.captain/config.toml`: Telegram, Discord, Signal, or Email. Non-core channel
sections such as Slack, WhatsApp, and Matrix are frozen out of the default
bridge policy.
...
```bash
captain skill install <skill-name-or-path>
```

Captain automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize Captain

```bash
captain init
```

This creates `~/.captain/` with a default `config.toml`.
...
**Captain** (`~/.captain/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
**Captain** (add to `~/.captain/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to Captain agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.captain/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | Captain |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.captain/config.toml` |
...
Tools were renamed between OpenClaw and Captain for consistency. The migration engine handles this automatically.

| OpenClaw Tool | Captain Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | Captain Tools |
|------------------|----------------|
...
| OpenClaw Name | Captain Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (Captain Protocol) peer-to-peer |
| **API server** | Basic REST | REST + WebSocket + SSE streaming |
| **WebChat UI** | Separate | Embedded in daemon |
| **Channel adapters** | Telegram, Discord | Active core: Telegram, Discord, Signal, Email; non-core compat is frozen |
| **Skills/Plugins** | npm packages | TOML + Python/WASM/Node.js skills; marketplace/ClawHub paths are frozen outside the active core release |
...
```bash
captain migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
captain skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.captain/channels_import.toml
# Copy the [channels.*] sections into ~/.captain/config.toml
```
