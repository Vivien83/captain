...
<tr><td><b>Trabajo duradero</b></td><td>Proyectos, objetivos, checkpoints, workflows y Live Runs en primer plano o desacopladas se persisten. Las salidas largas quedan fuera del contexto del modelo en pruebas acotadas, redactadas y verificadas por checksum que Captain puede leer, buscar o seguir después. Tras un reinicio, el trabajo incompleto queda visible como <code>interrupted</code> en vez de desaparecer o repetirse a ciegas. Los reintentos son explícitos, ligados al digest y auditados; un efecto interrumpido incierto requiere confirmación. Una API de operador autenticada expone metadatos y una cola acotada y saneada, y solo cancela un run activo respaldado por un handle de interrupción real. Control abre el mismo inventario privado desde su barra global, con filtros, actualización en vivo, colas acotadas mostradas solo como texto y cancelación únicamente cuando el runtime confirma que el run se puede interrumpir realmente. El TUI Ratatui completo ofrece el mismo inventario mediante <code>/runs</code>, rechaza colas obsoletas y exige una cancelación en dos pasos; el chat independiente y el terminal Web quedan limitados a metadatos acotados.</td></tr>
<tr><td><b>Pruebas antes de entregar</b></td><td>La conversación de solo lectura sigue siendo inmediata. Tras un trabajo con efectos, Captain evalúa receipts ordenados y redactados solo en hitos útiles y antes de entregar. Una mutación necesita una postcondición pertinente posterior; los jobs pendientes y controles fallidos siguen explícitos. Captain realiza como máximo dos correcciones dirigidas y después devuelve un resultado honestamente incompleto en vez de afirmar éxito. El estado de verificación sobrevive a paradas abruptas sin guardar razonamiento oculto ni repetir efectos inciertos.</td></tr>
<tr><td><b>Investigación basada en pruebas</b></td><td>La investigación agrupada busca y recupera fuentes independientes en paralelo, pero mantiene ordenada la verificación dependiente. Los fragmentos de búsqueda son solo para descubrir; una fuente pasa a ser citable únicamente tras leer la página con éxito y registrar URL final, fecha y SHA-256 del contenido. Para afirmaciones importantes, Captain vuelve a obtener las páginas citadas, rechaza enlaces inventados o citas ausentes, mide la procedencia declarada y genera la sección Fuentes desde URLs auditadas. La auditoría certifica recuperación y presencia literal, no la verdad semántica por mera afirmación.</td></tr>
...
<tr><td><b>Un solo Captain, en todas tus máquinas</b></td><td>Ejecuta un Hub completo y empareja Clientes ligeros de terminal, TUI o Desktop con las mismas sesiones, proyectos, memoria, modelo y aprobaciones. Los Nodos de ejecución opcionales actúan sobre espacios de trabajo locales solo mediante HTTPS 443 saliente, con proxy/CA empresarial, permisos explícitos, controles locales, confirmaciones duraderas y sin repetición ciega tras una desconexión.</td></tr>
...
Imagen Docker inmutable: `ghcr.io/vivien83/captain-agent-os:v0.1.0-alpha.15`;
...
Captain Full, Captain Console y Captain Node se publican para `aarch64` y
`x86_64` en macOS/Linux, además de `x86_64-pc-windows-msvc`. La release contiene
15 archivos, 15 sidecars SHA-256, 15 manifiestos componente/plataforma, seis
instaladores, un manifiesto agregado y procedencia ligada por checksum.

### Console ligera o Node de ejecución

Instala solo la Console multi-Captain, sin proveedor, memoria, canales, bucle
de agente ni daemon Full local:
...
captain-console pair --hub https://tu-captain.example
captain-console tui
```

El Node de workspace saliente es una instalación separada y opcional:

```bash
curl -fsSL https://github.com/Vivien83/captain/releases/download/v0.1.0-alpha.15/install-edition.sh \
  | CAPTAIN_EDITION=node CAPTAIN_VERSION=v0.1.0-alpha.15 bash
captain-node pair --hub https://tu-captain.example --workspace "$PWD"
captain-node service install
```

Ambas ediciones verifican el SHA-256 exacto y restauran el binario anterior si
falla la prueba de versión. El Node es de solo lectura por defecto; cualquier
mutación requiere un grant separado del Hub.
...
El perfil `vps` instala un servicio systemd, lo inicia y valida su salud. Si hay
un token de Telegram presente, Captain lo valida contra la API de
...
Captain puede gestionar un dominio HTTPS público de extremo a extremo. Crea
primero su registro DNS y define `CAPTAIN_DOMAIN` como arriba, o responde al
asistente interactivo. Captain mantiene la API en loopback, configura Caddy de
forma transaccional y termina solo cuando TLS, Control, la salud pública y la
paridad de versión están verificados.
...
Captain consulta su canal oficial de releases después del arranque y luego
cada 12 horas. Con un chat operador de Telegram exacto y una lista blanca
explícita, una tarjeta Rich ofrece **Actualizar**, **Recordar en 24 h** o
**Rechazar esta versión**. El plano de control procesa la decisión sin un turno
del modelo; nada se instala sin un clic. En el host se verifica el SHA-256 de
la release; Docker y las plataformas no compatibles siguen bajo control del
operador y se vuelven a comprobar. El estado duradero aparece en
`captain status` y `GET /api/status`.

En un dominio público configurado, Captain también comprueba cada cinco minutos
los puertos y la salud local/pública, DNS, TLS, el enrutamiento del proxy
inverso y la paridad exacta de versión. `captain doctor --full`, Status API y
Control leen la misma instantánea resistente a cortes y muestran acciones
concretas sin exponer errores de red sin filtrar.
...
| [Hub, Clientes y Nodos](docs/hub-clients-nodes.md) | Un Captain central, interfaces ligeras y ejecución local saliente |
...
| [Notas de la versión 0.1.0-alpha.15](docs/releases/v0.1.0-alpha.15.md) | Console multi-Captain, servicios Node nativos y bundles ligeros separados |
| [Notas de la versión 0.1.0-alpha.14](docs/releases/v0.1.0-alpha.14.md) | Un Hub, Clientes ligeros, Nodos salientes y trabajo distribuido resistente a fallos |
| [Notas de la versión 0.1.0-alpha.13](docs/releases/v0.1.0-alpha.13.md) | Verificación adaptativa, pruebas resistentes a fallos y actualizaciones de host reforzadas |
| [Notas de la versión 0.1.0-alpha.12](docs/releases/v0.1.0-alpha.12.md) | Live Runs duraderos, investigación fundamentada, artefactos verificados y dominios VPS gestionados |
...
CAPTAIN_VERSION=vX.Y.Z scripts/release-all.sh  # los 15 bundles en local
...
`release-all.sh` compila Full, Console y Node para dos objetivos macOS, dos
Linux y uno Windows, estrictamente un componente/objetivo cada vez; el
cross-build de Windows usa `cargo-xwin`, LLVM y NASM. Después de una gate de
release limpia, `publish-release-local.sh` valida 52 assets de host, añade una
declaración de procedencia SLSA v1 y su checksum (54 assets publicados), sube la rama actual,
...
por Git. No se confirma en el repositorio ni se añade a los 54 assets, y Docker
