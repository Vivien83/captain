/**
 * @captain/sdk — Official JavaScript client for the Captain Agent OS REST API.
 *
 * Usage:
 *   const { Captain } = require("@captain/sdk");
 *   const client = new Captain("http://localhost:3000");
 *
...
class Captain {
  /**
   * @param {string} baseUrl - Captain server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new CaptainError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  /** Create a new session. Set activate=false for an isolated client session. */
  async createSession(id, label, activate) {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new CaptainError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { Captain: Captain, CaptainError: CaptainError };
