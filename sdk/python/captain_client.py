"""
Captain Python Client — REST API client for controlling Captain remotely.

Usage:

    from captain_client import Captain

    client = Captain("http://localhost:3000")
...
Note: This is the REST API *client* library.
      For writing Python agents that run inside Captain, see captain_sdk.py instead.
"""
...
class _Resource:
    def __init__(self, client: "Captain"):
        self._c = client


class Captain:
    """Captain REST API client. Zero dependencies — uses only stdlib urllib."""
    ...
    def _request(self, method: str, path: str, body: Any = None) -> Any:
        ...
        try:
            ...
            raise CaptainError(f"HTTP {e.code}: {body_text}", e.code, body_text) from e
...
class _AgentResource(_Resource):
    ...
    def create_session(
        self, agent_id: str, label: Optional[str] = None, activate: bool = True
    ):
        return self._c._request(
            "POST",
            f"/api/agents/{agent_id}/sessions",
            {"label": label, "activate": activate},
        )
...
class _SessionResource(_Resource):
    ...
    def get(self, session_id: str):
        return self._c._request("GET", f"/api/sessions/{session_id}")
