...
assert.match(apiSource, /createSession:.*activate: false/s);
assert.match(apiSource, /openAgentWs\(agentId, sessionId, handlers\)/);
...
assert.match(shellSource, /api\.createSession\(agentId, \{ activate: false \}\)/);
assert.doesNotMatch(shellSource, /api\.switchSession\(/);
...
assert.match(chatSource, /openAgentWs\(agentId, sessionId/);
assert.match(chatSource, /api\.sessionEvents\(sessionId\)/);
