...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg CAPTAIN_SMOKE_FULL=1 .
...
ARG CAPTAIN_SMOKE_FULL=0
RUN if [ "$CAPTAIN_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
        ...
        bash -c ' \
            fail(){ echo "$*" >&2; exit 1; }; PROFILE=core; \
...
RUN if [ "$CAPTAIN_SMOKE_FULL" = "1" ] && [ -f "$HOME/.captain/bin/captain" ]; then \
        $HOME/.captain/bin/captain --version && \
        echo "PASS: captain binary works"; \
    else \
