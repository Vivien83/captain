...
- Keep each outbound message short enough to read on mobile. For long reports, send a compact summary first, then attach or chunk the details.
- Long turns emit a sparse visible heartbeat from the bridge. Telegram starts after ~2 min and then backs off heavily (about every 10 min) so mobile chats stay readable. Do not manually add duplicate "still working" messages unless the current step has genuinely changed.
...
- Long messages are silently chunked by the per-platform formatter (Telegram 4 KB, Discord 2 KB). The chunks are sent in order but each one is its own platform message.
