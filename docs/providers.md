...
Captain ships with a runtime model catalog backed by native Anthropic/Gemini
drivers and a universal OpenAI-compatible driver. The exact provider, model,
and alias inventory is code-defined and may change between builds; use
`captain models providers`, `captain models list`, and `captain models aliases`
to inspect the installed binary. This guide explains configuration, selection,
and model routing.
...
Captain auto-detects which providers have API keys configured at boot. Any model whose provider is authenticated becomes immediately available. Local providers (Ollama, vLLM, LM Studio) require no key at all.
...
| | |
|---|---|
| **Provider ID** | `codex` (`openai-codex` is accepted for compatibility) |
| **Driver** | Native Codex Responses API |
| **Auth** | ChatGPT subscription OAuth |
| **Setup** | `captain auth login codex` |
| **Live catalog** | `$CODEX_HOME/models_cache.json` or `~/.codex/models_cache.json` |

When at least one registered agent uses Codex, Captain refreshes the official
catalog 15 seconds after daemon startup and then once per hour. The first
successful refresh without a previous local cache establishes a silent
baseline. If a cache already exists, additions returned by the live endpoint
since that cache are treated as new. Known IDs and pending decisions are stored
durably, so restarts and repeated scans do not generate duplicate decisions.
The request declares catalog protocol compatibility as
`client_version=1.0.0`; this is a Codex capability marker required by the live
endpoint, not Captain's own release number.

A newly listed model is added to the in-memory catalog immediately, but Captain
never switches an agent automatically. Control shows a persistent notice with
**Conserver** and **Basculer** actions. A switch also requires choosing
**Nouvelle session** or **Résumé compact**, using the same provider-portable
model-switch rail as a manual switch. When Telegram has an active adapter and a
`default_chat_id`, Captain sends the same decision once to that chat; the exact
reply `Garder le modèle actuel` dismisses it, while `Basculer vers
codex/<model>` prepares the safe session choice.

If the live endpoint or OAuth refresh is temporarily unavailable, the last
valid cache remains usable. Captain records the failure in
`GET /api/models/updates` and retries at the next hourly scan.
...
**Notes:** Captain auto-discovers models from a running Ollama instance and merges them into the catalog with `Local` tier and zero cost. Any model you pull becomes usable immediately.
...
The catalog below is an operator reference for built-in models, sorted by provider. Pricing is per million tokens. Validate the exact installed inventory with `captain models list`.
...
- Local providers can add runtime-discovered models that vary per installation.

---

## Model Aliases

Aliases resolve to canonical model IDs. Aliases are case-insensitive. Validate
the exact installed alias map with `captain models aliases`.
...
Captain can automatically select the cheapest model capable of handling each query. This is configured per-agent via `ModelRoutingConfig`.
...
Captain tracks the cost of every LLM call and can enforce per-agent spending quotas.
...
### Inspect Codex Model Updates

```
GET /api/models/updates
```

Returns the durable Codex monitor state: whether Codex is active, baseline and
last-check status, newly discovered models awaiting a decision, recent
decisions, and the Codex agents eligible for a switch.
...
Switch one agent through the safe session rail:
...
`decision="switch"` is rejected unless the model is still pending and both
`agent_id` and `session_strategy` are explicit. Valid strategies are
`new_session` and `compact_session`.
...
Returns providers with auth status and model counts.
...
Lists providers with their authentication status.
