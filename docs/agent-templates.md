...
Captain ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
captain spawn orchestrator
captain spawn coder
captain spawn --template agents/writer/agent.toml
```
...
```bash
captain spawn security-auditor
...
> General-purpose assistant. The default Captain agent for everyday tasks, questions, and conversations.
...
```bash
captain spawn customer-support
...
```bash
captain spawn meeting-assistant
...
```bash
captain spawn personal-finance
...
captain spawn coder --name "backend-coder"

# Spawn from a TOML file path
captain spawn --template agents/custom/my-agent.toml

# List running agents
captain agents

# Send a message
captain message <agent-id> "Write a function to parse TOML files"

# Kill an agent
captain kill <agent-id>
```
