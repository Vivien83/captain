...
Skills are installed to `~/.captain/skills/` and made available to agents through the skill registry. Captain also ships with bundled skills compiled into the binary. Use `captain skill list` to inspect the installed runtime inventory.
...
Captain includes expert knowledge skills compiled into the binary (no installation needed). Use `captain skill list` for the exact runtime inventory:
...
| `tags` | array | No | Tags for local search and compatibility metadata |
...
1. Captain sends a JSON payload to the script's stdin:
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Captain-Skill/1.0"})
...
### Using the Captain Python SDK

For more advanced skills, use the Python SDK (`sdk/python/captain_sdk.py`):

```python
#!/usr/bin/env python3
from captain_sdk import SkillHandler
...
Captain can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
captain skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), Captain:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to Captain conventions.
4. Copies the skill to the Captain skills directory.
