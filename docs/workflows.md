...
The implementation lives in `captain-kernel/src/workflow.rs`. The workflow engine is decoupled from the kernel through closures -- it never directly owns or references the kernel, making it testable in isolation.
...
The trigger engine (`captain-kernel/src/triggers.rs`) provides event-driven automation. Triggers watch the kernel's event bus and automatically send messages to agents when matching events arrive.
...
#### `GET /api/workflows/:id` -- Inspect a workflow

Returns the complete workflow definition, including serialized steps. Invalid
UUIDs return `400`; unknown workflow ids return `404`.

#### `PUT /api/workflows/:id` -- Replace a workflow

Uses the same body shape as `POST /api/workflows` and replaces the definition
under the existing id.

#### `DELETE /api/workflows/:id` -- Remove a workflow

Deletes the workflow definition and returns `{"status":"removed"}`. Invalid
UUIDs return `400`; unknown workflow ids return `404`.
...
Returns only runs for the requested workflow. The route validates the workflow
UUID and orders results newest-first.
...
The authenticated Control app exposes workflows under
**Automation > Workflows**. Operators can list, create, inspect, run, review
scoped newest-first history, and delete workflows without leaving the six-hub
cockpit. Triggers, Crons, Approbations, and Webhooks are sibling Automation
tabs; they are not separate primary navigation entries.
...
All workflow and trigger CLI commands require a running Captain daemon.
...
```
captain trigger list [--agent-id <uuid>]
```
...
captain trigger create <agent-id> '"lifecycle"' --prompt "Lifecycle: {{event}}"

# Watch for a specific agent spawn
captain trigger create <agent-id> '{"agent_spawned":{"name_pattern":"coder"}}' --max-fires 1

# Watch for content containing "error"
captain trigger create <agent-id> '{"content_match":{"substring":"error"}}'
```
...
The `AgentScheduler` (in `captain-kernel/src/scheduler.rs`) tracks per-agent token usage with a rolling 1-hour window via `UsageTracker`. If an agent exceeds its `ResourceQuota.max_llm_tokens_per_hour`, the scheduler returns `CaptainError::QuotaExceeded`. The window resets automatically after 3600 seconds. This quota applies to all agent interactions, including those invoked by workflows.
...
- The `WorkflowEngine` is decoupled from `CaptainKernel`. The `execute_run` method takes two closures: `agent_resolver` (resolves `StepAgent` to `AgentId` + name) and `send_message` (sends a prompt to an agent and returns output + token counts). This design makes the engine testable without a live kernel.
