...
`v0.1.0-alpha.2`; pin it explicitly because GitHub's `/releases/latest`
...
curl -fsSL https://github.com/Vivien83/captain/releases/download/v0.1.0-alpha.2/install.sh \
  | CAPTAIN_VERSION=v0.1.0-alpha.2 CAPTAIN_PROFILE=desktop bash
```
...
```bash
CAPTAIN_IMAGE_TAG=v0.1.0-alpha.2 docker compose pull
CAPTAIN_IMAGE_TAG=v0.1.0-alpha.2 docker compose up -d --no-build
```
