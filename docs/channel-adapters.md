...
Captain's active messaging tier is deliberately small: **Telegram, Discord,
Signal, and Email**. These are the only external channels exposed as ready
operator surfaces while the core is being brought to Hermes-level reliability.

The repository still contains compatibility code and schema variants for a
larger adapter set, but those channels are frozen/cached from the product
surface. Do not treat them as ready setup paths until they have an explicit
reactivation tranche with live smoke coverage.

Active adapters share a common foundation: graceful shutdown via
`watch::channel`, exponential backoff on connection failures,
`Zeroizing<String>` for secrets, automatic message splitting for platform
limits, per-channel model/prompt overrides, DM/group policy enforcement,
per-user rate limiting, and output formatting.

## Table of Contents

- [Active Channels and Frozen Compatibility](#active-channels-and-frozen-compatibility)
...
## Active Channels and Frozen Compatibility
...
These variants may still exist in types, config compatibility, or historical
code paths, but their setup flows are hidden/frozen and they are not part of the
ready channel surface.
...
All channel configurations live in `~/.captain/config.toml` under the `[channels]` section. Each channel is a subsection:
...
- `bot_token_env` / `token_env` -- The environment variable holding the bot/access token. Captain reads the token from this env var at startup. All secrets are stored as `Zeroizing<String>` and wiped from memory on drop.
- `default_agent` -- The agent name (or ID) that receives messages when no specific routing applies.
- `allowed_users` -- Optional list of platform user IDs allowed to interact.
  For active inbound adapters, an empty allowlist is deny-by-default; use
  `["*"]` only when deliberate open access is desired.
- `overrides` -- Optional per-channel behavior overrides (see [Channel Overrides](#channel-overrides) below).

### Environment Variables Reference (Active Channels)

| Channel | Required Env Vars |
|---------|-------------------|
| Telegram | `TELEGRAM_BOT_TOKEN` |
| Discord | `DISCORD_BOT_TOKEN` |
| Signal | signal-cli local service/config |
| Email | `EMAIL_PASSWORD` |

Env vars shown in frozen compatibility tables are implementation references,
not active setup guidance.
...
The `formatter` module (`captain-channels/src/formatter.rs`) converts Markdown output from the LLM into platform-native formats:
...
The `ChannelRateLimiter` (`captain-channels/src/rate_limiter.rs`) uses a `DashMap` to track per-user message counts. When `rate_limit_per_user` is set on a channel's overrides, the limiter enforces a sliding-window cap of N messages per minute. Excess messages receive a polite rejection.
...
user_id = "@captain-bot:matrix.org"
default_agent = "assistant"
```
...
To add support for a new messaging platform, implement the `ChannelAdapter` trait. The trait is defined in `crates/captain-channels/src/types.rs`.
...
Create `crates/captain-channels/src/myplatform.rs`:
...
In `crates/captain-channels/src/lib.rs`:
...
In `crates/captain-api/src/channel_bridge.rs`, add initialization logic for your adapter alongside the existing adapters.

### 4. Add Config Support

In `captain-types`, add a config struct:
...
In `crates/captain-cli/src/main.rs`, add a case to `cmd_channel_setup` with step-by-step instructions for your platform.
