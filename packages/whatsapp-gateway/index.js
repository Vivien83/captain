...
const CAPTAIN_URL = (process.env.CAPTAIN_URL || 'http://127.0.0.1:4200').replace(/\/+$/, '');
const DEFAULT_AGENT = process.env.CAPTAIN_DEFAULT_AGENT || 'assistant';
...
async function startConnection() {
  ...
  // Incoming messages → forward to Captain
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to Captain agent
      try {
        const response = await forwardToCaptain(text, phone, pushName, metadata);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to Captain API, return agent response
// ---------------------------------------------------------------------------
function forwardToCaptain(text, phone, pushName, metadata) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${CAPTAIN_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Captain API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by Captain for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] Captain URL: ${CAPTAIN_URL}`);
